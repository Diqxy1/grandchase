...
KGCPlayerAmy::KGCPlayerAmy(void)
{
}
...
int KGCPlayerAmy::OnCombo(PLAYER* pPlayer, bool bKeyProc)
{
    if( bKeyProc )
    {
        ...
        if( g_MyD3D->MyCtrl->k_Fresh_Right_Dash || g_MyD3D->MyCtrl->k_Fresh_Left_Dash )
        {
            ...
            else if( pPlayer->uiMotion != pPlayer->GetCurFormTemplate().ATK1 )
            {
                if( g_MyD3D->MyCtrl->k_Fresh_Right_Dash )
                    pPlayer->bIsRight = true;
                else pPlayer->bIsRight = false;
        ...
        if( pPlayer->GetCurFormTemplate().WIN == MID_AMY1_WINPOS )
            if( pPlayer->CheckSkill( SID_AMY1_FIGHT_COMBOAPPLICATION ) )         
                if( g_MyD3D->MyCtrl->k_Fresh_Up ) 
                    pPlayer->Change_Motion( MID_AMY1_SKILL_COMBO_SPECIAL );
...
int KGCPlayerAmy::OnRun(PLAYER* pPlayer, bool bKeyProc)
{
    if(bKeyProc)
    {
        if( g_MyD3D->MyCtrl->k_Fresh_Punch )
    ...
    if( pPlayer->uiMotion == pPlayer->GetCurFormTemplate().RUN )
    {
        if( pPlayer->cFrame >= (int)pPlayer->GetNowMotionFRM()->GetNum_Frame() - 1 )
        {
            ...
            if( true == pPlayer->IsContact )
                pPlayer->SetPlayerState( MID_AMY1_RUN_TO_WAIT );
            else
...
int KGCPlayerAmy::OnJump(PLAYER* pPlayer, bool bKeyProc)
{
    if(bKeyProc)
    {
...
int KGCPlayerAmy::OnDash(PLAYER* pPlayer, bool bKeyProc)
{
...
int KGCPlayerAmy::OnJumpDash(PLAYER* pPlayer, bool bKeyProc)
{
    if(bKeyProc)
    {
        if( g_MyD3D->MyCtrl->k_Up )
...
int KGCPlayerAmy::OnDashAtk(PLAYER* pPlayer, bool bKeyProc)
{
    if( bKeyProc )
    {
        if( g_MyD3D->MyCtrl->k_Fresh_Punch )
    ...
    if( pPlayer->cFrame >= (int)pPlayer->GetNowMotionFRM()->GetNum_Frame() - 1 )
    {
        ...
        {
            if( true == pPlayer->IsContact )
                pPlayer->SetPlayerState( MID_AMY1_DASH_ATK1_TO_WAIT );
            else
        ...
        {
            if( true == pPlayer->IsContact )
                pPlayer->SetPlayerState( MID_AMY1_DASH_ATK2_TO_WAIT );
            else
...
int KGCPlayerAmy::OnSpecial1Atk(PLAYER* pPlayer, bool bKeyProc)
{
    if( bKeyProc )
    {
...
int KGCPlayerAmy::OnSpecial2Atk(PLAYER* pPlayer, bool bKeyProc)
{
    ...
    if( pPlayer->cFrame >= (int)pPlayer->GetNowMotionFRM()->GetNum_Frame() - 1 )
    {
        if( pPlayer->IsContact )
        {
            pPlayer->SetPlayerState( MID_AMY1_UNIQUE_ATK2_TO_WAIT );
            pPlayer->cFrame = 0;
...
int KGCPlayerAmy::OnDoubleJump(PLAYER* pPlayer, bool bKeyProc)
{
    if(bKeyProc)
    {
        //방향 바꾸기 가능하고 점프 어텍으로 갈수 있음
...
int KGCPlayerAmy::OnDoubleDash(PLAYER* pPlayer, bool bKeyProc)
{
    if(bKeyProc)
    {
        if( g_MyD3D->MyCtrl->k_Up )
    ...
    {
        if( true == pPlayer->IsContact )
            pPlayer->SetPlayerState( MID_AMY1_DOUBLE_DASH_TO_WAIT );
        else
...
int KGCPlayerAmy::OnDashToWaitJump(PLAYER* pPlayer, bool bKeyProc)
{
    if( bKeyProc )
    {
        if( (true == pPlayer->bIsRight && true == g_MyD3D->MyCtrl->k_Fresh_Right_Dash) ||
...
int KGCPlayerAmy::OnDoubleDashToWaitJump(PLAYER* pPlayer, bool bKeyProc)
{
    if( bKeyProc )
    {
        if( g_MyD3D->MyCtrl->k_Fresh_Punch )
...
int KGCPlayerAmy::OnRunToWaitJump(PLAYER* pPlayer, bool bKeyProc)
{
    if( bKeyProc )
    {
        if( g_MyD3D->MyCtrl->k_Fresh_Punch )
...
int KGCPlayerAmy::OnRunToDoubleJump(PLAYER* pPlayer, bool bKeyProc)
{
    if(bKeyProc)
    {
        //방향 바꾸기 가능하고 점프 어텍으로 갈수 있음
...
int KGCPlayerAmy::OnSpecialAtk2ToWaitJump(PLAYER* pPlayer, bool bKeyProc)
{
    if( pPlayer->cFrame >= (int)pPlayer->GetNowMotionFRM()->GetNum_Frame() - 1 )
    {
        if( pPlayer->IsContact )
...
int KGCPlayerAmy::OnDashAtk1ToWaitJump(PLAYER* pPlayer, bool bKeyProc)
{
    if( bKeyProc )
    {
        if( g_MyD3D->MyCtrl->k_Fresh_Punch )
...
int KGCPlayerAmy::OnDashAtk2ToWaitJump(PLAYER* pPlayer, bool bKeyProc)
{
    if( pPlayer->cFrame >= (int)pPlayer->GetNowMotionFRM()->GetNum_Frame() - 1 )
    {
        if( pPlayer->IsContact )
