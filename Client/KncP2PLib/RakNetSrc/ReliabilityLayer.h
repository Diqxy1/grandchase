...
public:
	 ...
	 */
	bool HandleSocketReceiveFromConnectedPlayer( char *buffer, int length, DWORD dwUID_ );
	 ...
	 */
    bool Send( RakNet::BitStream *bitStream, PacketPriority priority, PacketReliability reliability, unsigned char orderingChannel, bool makeDataCopy, int MTUSize, std::vector<DWORD> vecUID_ = std::vector<DWORD>() );
    ...
    void SetAlive(){ deadConnection = false; }
...
private:
	 ...
	 * 
	 */
	bool GenerateFrame( RakNet::BitStream *output, int MTUSize, bool *reliableDataSent, unsigned long time, const PlayerID& kPlayerID_ );
	...
	bool RemovePacketFromResendQueueAndDeleteOlderReliableSequenced( PacketNumberType packetNumber, DWORD dwUID_ );
    ...
    void SplitPacketAndDeleteOriginal( InternalPacket *internalPacket, int MTUSize, std::vector<DWORD> vecUID_ );
...
public:
    //====================================================================================
    // 2006.07.05 : Asirion
    // ms_kRelayServerID    :   RelayServer주소
    // ms_dwUID             :   Local User UID
    // m_vecUID             :   담당 해야할 User의 UID vector
    // m_bRelayMain         :   RelayServer로 Packet을 Send할 수 있는 녀석이냐? 
    //                          false이면 ack만 보낸다.
    static PlayerID     ms_kRelayServerID;
    static DWORD        ms_dwUID;
    std::vector<DWORD>  m_vecUID;
    bool                m_bRelayMain;
    std::vector<DWORD>  m_vecLastSendedUID;

    int                 m_iCntFailRead;
    int                 m_iCntFailReadCompressed;
    int                 m_iCntFailReadAlignedBytes;

    void WriteRelayPacketHeader( RakNet::BitStream* pkBitStream_, std::vector<DWORD>* pvecUID_ );
};
