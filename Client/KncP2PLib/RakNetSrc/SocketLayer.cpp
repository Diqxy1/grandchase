...
bool SocketLayer::RecvFrom( SOCKET s, RakPeer *rakPeer, int *errorCode )
{
	...
	else
	{
		...
		*errorCode = 0;
		...
		{
            m_ulBinaryIP_From_WSAECONNRESET = sa.sin_addr.s_addr;
            m_usPort_WSAECONNRESET = ntohs( sa.sin_port );
#if defined(_DEBUG)
			printf( "A previous Recv operation resulted in an ICMP Port Unreachable message.\n" );
#endif			
			*errorCode = dwIOError;
			return false;
		}
...
int SocketLayer::SendTo( SOCKET s, char *data, int length, unsigned long binaryAddress, unsigned short port )
{
	...
	while ( len == 0 );
    //s_iSendLength += length;
    //if ( 1000 < timeGetTime() - s_dwTime )
    //{
    //    printf( "Send Packet Length Per Second : %d\n", s_iSendLength );
