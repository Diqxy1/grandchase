/* -*- mode: c++; c-file-style: carnet; tab-always-indent: nil; -*- */
/**
 * @file 
...
//====================================================================================
// 2006.07.06 : Asirion
// BYTE_FOR_RELAYSERVER = sizeof( bool ) + sizeof( DWORD ) + sizeof( DWORD )
// bRelayMain + SenderUID + ReceiverUID
#define                    BYTE_FOR_RELAYSERVER                 9
...
RakPeer::RakPeer()
{
    ...
    //====================================================================================
    // 2006.07.18 : Asirion
    // RelayServer 지원 모듈 추가.
    m_kRelayServerID = UNASSIGNED_PLAYER_ID;
    m_dwUID = 0;
}
...
//////////////////////////////////////////////////////////////////////////
//                          2005-05-18 이창우 수정
//                    이미 Connection을 맺은 Player에 
//                  또다시 Connection을 요청할경우 오류
//////////////////////////////////////////////////////////////////////////
bool RakPeer::Connect( char* host, unsigned short remotePort, char* passwordData, int passwordDataLength )
{
    ...
    if ( GetRemoteSystemFromPlayerID( playerID ) != 0 )
        return false;
    if ( GetRelayServerID() == playerID )
        return false;
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::Disconnect( unsigned long blockDuration, unsigned long userData )
{
	unsigned i;
    ...
    //====================================================================================
    // 2006.07.08 : Asirion
    // TODO : CloseConnection( remoteSystemList[ i ].playerId, false, blockDuration );
    // 의 false부분 처리해야한다.
    for ( i = 0; i < maxPeers; i++ )
    {
        // CloseConnection uses maximumNumberOfPeers
        CloseConnection( remoteSystemList[ i ].playerId, false, blockDuration );
    }
    RakPeer::RemoteSystemStruct* pRemote = GetRelayServerRemoteSystem();
    if( pRemote != NULL )
        pRemote->reliabilityLayer.m_vecUID.clear();
	...
	*/
    ...
    {
        m_iCntFailRead += remoteSystemList[i].reliabilityLayer.m_iCntFailRead;
        m_iCntFailReadCompressed += remoteSystemList[i].reliabilityLayer.m_iCntFailReadCompressed;
        m_iCntFailReadAlignedBytes += remoteSystemList[i].reliabilityLayer.m_iCntFailReadAlignedBytes;
    }
	
    delete [] remoteSystemList;
...
bool RakPeer::SendToRelayServer( RakNet::BitStream * bitStream, PacketPriority priority, PacketReliability reliability, char orderingChannel, std::vector<DWORD> vecUID_  )
{
    RakPeer::RemoteSystemStruct* pRemote = GetRelayServerRemoteSystem();
    if ( pRemote == NULL )
        return false;
    ...
    if ( outputTree )
    {
        ...
        outputTree->EncodeArray( bitStream->GetData(), bitStream->GetNumberOfBytesUsed(), &bitStreamCopy );
        compressedBytesSent += bitStreamCopy.GetNumberOfBytesUsed();
        return pRemote->reliabilityLayer.Send( &bitStreamCopy, priority, reliability, orderingChannel, true, MTUSize, vecUID_ );
    }
    else
    {
        return pRemote->reliabilityLayer.Send( bitStream, priority, reliability, orderingChannel, true, MTUSize, vecUID_);
    }
    return true;
}
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
//====================================================================================
// 2006.07.06 : Asirion
// 1. target == RelayServerID : RelayMain에서 UID삭제
//                              RelayMain이 아닌 remoteSystemList삭제.
// 2. target != RelayServerID : target과 remoteSystemList.playerID만 비교한다.
void RakPeer::CloseConnection( PlayerID target, bool sendDisconnectionNotification, unsigned long blockDuration, DWORD dwUID_ )
{
	if ( remoteSystemList == 0 || endThreads == true )
		return ;
    //====================================================================================
	...
	if ( sendDisconnectionNotification )
        SendDisconnectNot( dwUID_, target );
	
    if ( target == UNASSIGNED_PLAYER_ID )
        return;
	rakPeerMutexes[ RakPeer::remoteSystemList_Mutex ].Lock();
    RakPeer::RemoteSystemStruct* pRemoteSystem = NULL;
    if ( target == GetRelayServerID() )
    {
        pRemoteSystem = GetRemoteSystemFromPlayerID( dwUID_, false );
    }
    else
    {
        pRemoteSystem = GetRemoteSystemFromPlayerID( target );
    }
    if ( pRemoteSystem == NULL )
    {
        rakPeerMutexes[ RakPeer::remoteSystemList_Mutex ].Unlock();
        return;
    }

    unsigned stopWaitingTime = RakNet::GetTime() + blockDuration;
    while ( RakNet::GetTime() < stopWaitingTime )
    {
        if ( pRemoteSystem->reliabilityLayer.GetStatistics()->messageSendBuffer[ SYSTEM_PRIORITY ] == 0 )
            break;					
        #ifdef _WIN32
            Sleep( 0 );
        #else
    ...
    pRemoteSystem->playerId = UNASSIGNED_PLAYER_ID; // This one line causes future incoming packets to go through the reliability layer
    pRemoteSystem->reliabilityLayer.Reset();			
...
//====================================================================================
// 2006.07.06 : Asirion
void RakPeer::SendDisconnectNot( DWORD dwUID_, const PlayerID& kPlayerID_ )
{
    UINT uiPacketSize = sizeof(unsigned char) + sizeof(DWORD);
    unsigned char* pSendData = new unsigned char[uiPacketSize];
    unsigned char* pBuffer = pSendData;
    pBuffer[0] = ID_DISCONNECTION_NOTIFICATION;
    pBuffer += sizeof( unsigned char );
    memcpy( pBuffer, &dwUID_, sizeof( unsigned long) );
    Send( ( char* ) pSendData, uiPacketSize, SYSTEM_PRIORITY, RELIABLE, 0, kPlayerID_, false );
    delete [] pSendData;
    lastUserUpdateCycle = RakNet::GetTime();
}
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
// 2005-09-09 Asirion
// Ping모듈이 사실상 필요도 없을뿐더러 바뀐 Send함수와 맞지 않는다.
// ID_PONG, ID_PING, Ping 함수등을 모두 막아 놓았다.
...
PlayerID RakPeer::GetRelayServerID( void )
{
    return m_kRelayServerID;
}
void RakPeer::SetupRelayServer( PlayerID &kRelayServerID, DWORD dwUID_ )
{
    if ( m_kRelayServerID != UNASSIGNED_PLAYER_ID )
        return;
    m_kRelayServerID = kRelayServerID;
    RakPeer::RemoteSystemStruct* pRemote = AssignPlayerIDToRemoteSystemList( m_kRelayServerID, NULL, false, std::vector<DWORD>(), true );
    m_dwUID = pRemote->reliabilityLayer.ms_dwUID  = dwUID_;
    pRemote->reliabilityLayer.ms_kRelayServerID  = kRelayServerID;
}
void RakPeer::SetRegisterToRelayServer()
{
    Packet * packet = PacketPool::Instance() ->GetPointer();
    packet->data = new unsigned char [ sizeof(char) ];
    packet->data[0] = ID_REPORT_PUBLIC_IP;
    packet->playerId = GetRelayServerID();
    packet->length = sizeof(char);
...
std::vector<DWORD> RakPeer::GetLocalIP()
{
    std::vector<DWORD> vecIP;
    unsigned int iNumberOfAddress = GetNumberOfAddresses();
    for ( int i = 0; i < (int)iNumberOfAddress; i++ )
    {
        vecIP.push_back( ::inet_addr( GetLocalIP( (unsigned int) i )) );
    }
    return vecIP;
}
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
void RakPeer::AdvertiseSystem( char *host, unsigned short remotePort )
{
    //====================================================================================
    // 2006.07.04 : Asirion
    // Domain인 결우 IPv4형태로 바꿈.
	if ( host[ 0 ] < '0' || host[ 0 ] > '2' )
	{
		host = ( char* ) SocketLayer::Instance() ->DomainNameToIP( host );
	}
    START_LOG( clog2, "AdvertiseSystem" ) << BUILD_LOG( host ) << BUILD_LOG( remotePort );
    PlayerID playerID;
    IPToPlayerID( host, remotePort, &playerID);
    if ( playerID == GetRelayServerID() )
        return;
	unsigned char c = ID_ADVERTISE_SYSTEM;
    SocketLayer::Instance() ->SendTo( connectionSocket, ( char* ) & c, sizeof( c ), ( char* ) host, remotePort );
}
void RakPeer::AdvertiseSystem( char *host, unsigned short remotePort, DWORD dwUID_ )
{
    //====================================================================================
    // 2006.07.04 : Asirion
    // Domain인 결우 IPv4형태로 바꿈.
    ...
    if ( playerID == GetRelayServerID() )
        return;
    unsigned char* pSendData = new unsigned char[sizeof(unsigned char) + sizeof(DWORD)];
    unsigned char* pBuffer = pSendData;
    pBuffer[0] = ID_ADVERTISE_SYSTEM;
    pBuffer += sizeof( unsigned char );
    memcpy( pBuffer, &dwUID_, sizeof( DWORD ) );
    SocketLayer::Instance() ->SendTo( connectionSocket, (char*)pSendData, sizeof(unsigned char) + sizeof(DWORD), 
                                      ( char* ) host, remotePort );
    delete [] pSendData;
}
void RakPeer::SendEchoToServer( DWORD dwServerIP_, u_short usServerPort_, char* strUserName_, UINT uiLength )
{
    SocketLayer::Instance() ->SendTo( connectionSocket, strUserName_, (int)uiLength, dwServerIP_, usServerPort_ );
}
void RakPeer::RegisterToRelayServer()
{
    RakNet::BitStream kBitStream;
    kBitStream.Reset();
    RemoteSystemStruct* pRemote = GetRelayServerRemoteSystem();
    kBitStream.Write( (unsigned char)1 );
    kBitStream.Write( pRemote->reliabilityLayer.ms_dwUID );
    kBitStream.Write( (unsigned char)1 );
    kBitStream.Write( pRemote->reliabilityLayer.ms_dwUID );
    kBitStream.Write( pRemote->reliabilityLayer.ms_dwUID );
	for ( int i = 0; i < 3; i++ )
	{
		SocketLayer::Instance() ->SendTo( connectionSocket, (char*)kBitStream.GetData(), kBitStream.GetNumberOfBytesUsed(), 
										  pRemote->reliabilityLayer.ms_kRelayServerID.binaryAddress, 
										  pRemote->reliabilityLayer.ms_kRelayServerID.port );
	}
}
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
// Description:
...
PlayerID RakPeer::IPToPlayerID( const char* host, unsigned short remotePort )
{
    PlayerID kPlayerID;
    IPToPlayerID( host, remotePort, &kPlayerID );
    return kPlayerID;
}
PlayerID RakPeer::IPToPlayerID( const DWORD dwIP, const USHORT usPort_ )
{
    PlayerID kPlayerID;
    kPlayerID.binaryAddress = dwIP;
    kPlayerID.port = usPort_;
    return kPlayerID;
}
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
RakPeer::RemoteSystemStruct *RakPeer::GetRemoteSystemFromPlayerID( DWORD dwUID_, bool bRelayMain_ ) const
{
    ...
    if ( bRelayMain_ == true )
    {
        return GetRelayServerRemoteSystem();
    }
    else
    {
        for ( unsigned i = 0; i < maximumNumberOfPeers; i++ )
        {
            ...
            if ( std::find( remoteSystemList[ i ].reliabilityLayer.m_vecUID.begin(),
                 remoteSystemList[ i ].reliabilityLayer.m_vecUID.end(), 
                dwUID_ ) != remoteSystemList[ i ].reliabilityLayer.m_vecUID.end() )
            {
                return remoteSystemList + i;
            }                  
        }
...
RakPeer::RemoteSystemStruct *RakPeer::GetRelayServerRemoteSystem() const
{
    for ( unsigned i = 0; i < maximumNumberOfPeers; i++ )
    {
        if ( remoteSystemList[ i ].reliabilityLayer.m_bRelayMain == true )
        {
            return remoteSystemList + i;
        }
    }
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
RakPeer::RemoteSystemStruct * RakPeer::AssignPlayerIDToRemoteSystemList( PlayerID playerId, unsigned char *AESKey, bool setAESKey, std::vector<DWORD> vecUID_, bool bRelayMain_ )
{
	...
	for ( i = 0; i < maximumNumberOfPeers; i++ )
    {
        if ( remoteSystemList[ i ].playerId == playerId && playerId != m_kRelayServerID )
        {
            rakPeerMutexes[ RakPeer::remoteSystemList_Mutex ].Unlock();
            return 0;
        }
	...
	for ( i = 0; i < maximumNumberOfPeers; i++ )
	{
		if ( remoteSystemList[ i ].playerId == UNASSIGNED_PLAYER_ID )
		{
            ...
            ( remoteSystemList[ i ] ).playerId = playerId;
            ( remoteSystemList[ i ] ).reliabilityLayer.m_vecUID  = vecUID_;
            ( remoteSystemList[ i ] ).reliabilityLayer.m_bRelayMain = bRelayMain_;
            if ( bRelayMain_ ==  true )
            {
                ( remoteSystemList[ i ] ).playerId = GetRelayServerID();
            }
...
#endif
{
	PlayerID playerId = rakPeer->IPToPlayerID( binaryAddress, port );
    //====================================================================================
    // 2006.07.05 : Asirion
    // 1. data에서 보낸 User의 UID Field분리.
    // 2. 해당 UID를 가진 remoteSystemList가 없으면 추가.
    // 3. RelayServer로 Packet을 전송할 수 없는 remoteSystem이 없으면 추가.
    // 4. Handler로 넘겨준다.
    RakPeer::RemoteSystemStruct *remoteSystem = NULL;
    bool  bRelayMain = true;
    DWORD dwUID = 0;
    if ( playerId == rakPeer->GetRelayServerID() )
    {
        memcpy( &bRelayMain, data, sizeof(bool) );
        memcpy( &dwUID, data + sizeof(bool), sizeof(DWORD) );
        if ( dwUID == rakPeer->m_dwUID )
        {
            rakPeer->SetRegisterToRelayServer();
            return;
        }
        remoteSystem = rakPeer->GetRemoteSystemFromPlayerID( dwUID, !bRelayMain );
        //====================================================================================
        // 2006.07.06 : Asirion    
        // remoteSystem이 NULL이라 함은 처음으로 RelayServer를 통해서 Ack가 아닌 Packet이
        // 도착하였다는 말이다. 즉 RelayMain이 false일 수 없다.
        if ( remoteSystem == NULL && bRelayMain == true )
        {
            std::vector< DWORD > vecUID;
            vecUID.push_back( dwUID );
            remoteSystem = rakPeer->AssignPlayerIDToRemoteSystemList( rakPeer->GetRelayServerID(), NULL, false, vecUID );
        }
        data += BYTE_FOR_RELAYSERVER;
        length -= BYTE_FOR_RELAYSERVER;
    }
    else
    {
	...
	if ( remoteSystem )
	{
        ...
        //====================================================================================
        // 2006.07.05 : Asirion
        // 접속이 되어 있는 상태에서는 ID_ADVERTISE_SYSTEM 를 무시한다.
        if ( ( unsigned char ) ( data ) [ 0 ] == ID_ADVERTISE_SYSTEM && 
             ( length == sizeof( unsigned char ) || length == sizeof( unsigned char ) + sizeof( unsigned long ) ) )
        {
            return;
        }
		if ( remoteSystem->reliabilityLayer.HandleSocketReceiveFromConnectedPlayer( data, length, dwUID ) == false )
		{
					...
					length == 1 + 20 + sizeof( RSA_BIT_SIZE ) )
				{
					...
					if ( ( unsigned char ) ( data ) [ 0 ] == ID_CONNECTION_REQUEST_ACCEPTED && length == sizeof( ConnectionAcceptStruct ) )
					{
						...
						if ( requestedConnection )
						{
                            ...
                            // 2005-07-20 KncP2P 공유기 부분 수정.
							// playerId.port = cas->externalID.port;
							playerId.port = port;
...
bool RakPeer::RunUpdateCycle( void )
{
	...
	do
	{
		...
		if ( gotData == false )
		{
			...
			{
                PlayerID playerID;
                playerID.binaryAddress = SocketLayer::Instance()->m_ulBinaryIP_From_WSAECONNRESET;
                playerID.port = SocketLayer::Instance()->m_usPort_WSAECONNRESET;
				PushPortRefused( playerID );
				//closesocket(peer->connectionSocket);
	...
	for ( remoteSystemIndex = 0; remoteSystemIndex < maximumNumberOfPeers; ++remoteSystemIndex )
	{
		if ( remoteSystemList[ remoteSystemIndex ].playerId != UNASSIGNED_PLAYER_ID )
		{
			...
			if ( remoteSystem->reliabilityLayer.IsDeadConnection() )
			{
				...
				packet->data = new unsigned char [ sizeof( char ) + remoteSystem->staticData.GetNumberOfBytesUsed() + sizeof(DWORD)];
				...
				packet->length = sizeof( char ) + remoteSystem->staticData.GetNumberOfBytesUsed() + sizeof(DWORD);
				packet->bitSize = packet->length * 8;
				...
				( incomingPacketQueue ).push( packet );
				incomingQueueMutex.Unlock();
                remoteSystem->reliabilityLayer.SetAlive();
...
//				CloseConnection( remoteSystem->playerId, false, 0L );
			...
			//{
			//	remoteSystem->nextPingTime = time + 3000;
			...
			while ( bitSize > 0 )
			{
				...
				*/
							...
							if ( ( unsigned char ) data[ 0 ] == ID_DISCONNECTION_NOTIFICATION )
							{
								...
								{
                                    packet->data = new unsigned char [ byteSize + remoteSystem->staticData.GetNumberOfBytesUsed() ];
                                    memcpy( packet->data, data, byteSize );
                                    memcpy( packet->data + byteSize, remoteSystem->staticData.GetData(), remoteSystem->staticData.GetNumberOfBytesUsed() );

                                    packet->length = byteSize + remoteSystem->staticData.GetNumberOfBytesUsed();
                                    packet->bitSize = byteSize * 8 + remoteSystem->staticData.GetNumberOfBitsUsed();
