...
public:
	...
	virtual ~RakPeer();
	/**
	*  incomingPacketQueue Size를 받아 온다.
	*/
	UINT GetIncomingPacketQueueSize(){ return incomingPacketQueue.size(); }
	/*
	 * --------------------------------------------------------------------------------------------
	 ...
	 */
	virtual void Disconnect( unsigned long blockDuration, unsigned long userData = 0x00000000 );
	/**
    ...
    bool SendToRelayServer( RakNet::BitStream * bitStream, PacketPriority priority, PacketReliability reliability, char orderingChannel, std::vector<DWORD> vecUID_ );
    ...
    //====================================================================================
    // 2006.07.06 : Asirion
	void CloseConnection( PlayerID target, bool sendDisconnectionNotification, unsigned long blockDuration, DWORD dwUID_ = 0 );
    void SendDisconnectNot( DWORD dwUID_, const PlayerID& kPlayerID_ );
    ...
    PlayerID GetRelayServerID( void );
    void     SetupRelayServer( PlayerID &kRelayServerID, DWORD dwUID_ );
    ...
    PlayerID IPToPlayerID( const char* host, unsigned short remotePort );
    PlayerID IPToPlayerID( const DWORD dwIP, const USHORT usPort_ );
	 ...
	 * @param remotePort Which port to connect to on the remote machine.
	 */
    void RegisterToRelayServer();
    void SetRegisterToRelayServer();
	void AdvertiseSystem( char *host, unsigned short remotePort );
    void AdvertiseSystem( char *host, unsigned short remotePort, DWORD dwUID_ );
    void SendEchoToServer( DWORD dwServerIP_, u_short usServerPort_, char* strUserName_, UINT uiLength );
...
public:
    ...
    RemoteSystemStruct *GetRemoteSystemFromPlayerID( DWORD dwUID_, bool bRelayMain_ ) const;
    RemoteSystemStruct *GetRelayServerRemoteSystem() const;
	/**
	 ...
	 */
    RemoteSystemStruct * AssignPlayerIDToRemoteSystemList( PlayerID playerId, unsigned char *AESKey, bool setAESKey, std::vector<DWORD> vecUID_ = std::vector<DWORD>(), bool bRelayMain_ = false );
	/**
    ...
    //====================================================================================
    // 2006.07.18 : Asirion
    // RelayServer에 나의 UID를 등록 하는 모듈 작성.
    DWORD   m_dwUID;

    int                 m_iCntFailRead;
    int                 m_iCntFailReadCompressed;
    int                 m_iCntFailReadAlignedBytes;
