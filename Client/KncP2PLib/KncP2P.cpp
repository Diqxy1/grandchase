...
KncP2P::KncP2P()
{
    ...
    m_usDefaultLocalPort = DEFAULT_PORT;
...
bool KncP2P::ConnectP2P( DWORD dwUID_ )
{
	_JIF( m_bInit, SET_ERROR( KNC_KNCP2P_INVALID_PTR );return false );
	_JIF( m_pRakPeer, SET_ERROR( KNC_KNCP2P_INVALID_PTR );return false );
	_JIF( m_pPeerManager, SET_ERROR( KNC_KNCP2P_INVALID_PTR );return false );
	...
	m_pThreadRecv->Connect( dwUID_ );
	return true;
}

void KncP2P::SetConnectionUID( DWORD dwUID )
{
	m_dwConnectionUID = dwUID;
	m_pThreadRecv->SetConnectionUID( dwUID );
}
...
void KncP2P::Send( void* pPakcet_, int iPacketSize_, DWORD dwUID_, SendReliability optReliability_ )
{
	...
	s_kGameMsg.dwConnectionUID = m_dwConnectionUID;
...
DWORD KncP2P::GetAvgPingTime( bool bRelay )
{
    _JIF( m_pPeerManager, SET_ERROR( KNC_THREADRECV_INVALID_PTR );return -1; );
    return m_pPeerManager->GetAvgPingTime( bRelay );
}
...
DWORD KncP2P::GetAvgPartPingTime()
{
    _JIF( m_pPeerManager, SET_ERROR( KNC_THREADRECV_INVALID_PTR );return -1; );
    return m_pPeerManager->GetAvgPartPingTime();
}

DWORD KncP2P::GetAvgPartPingTime( DWORD dwUID_ )
{
    _JIF( m_pPeerManager, SET_ERROR( KNC_THREADRECV_INVALID_PTR );return -1; );
    return m_pPeerManager->GetAvgPartPingTime(dwUID_);
}

bool KncP2P::IsRelay( DWORD dwUID_ )
{
    _JIF( m_pPeerManager, SET_ERROR( KNC_THREADRECV_INVALID_PTR );return -1; );
    return m_pPeerManager->IsRelay(dwUID_);
}
...
KncPeerManager* KncP2P::GetPeerManager()
{
	if( m_pPeerManager )
	{
	...
	assert( false && L"PeerManager Is Null!" );
	return NULL;
}

void KncP2P::SetRelayOnly( bool bRelay_ )
{
	if( m_pThreadRecv )
		m_pThreadRecv->SetRelayOnly( bRelay_ );
