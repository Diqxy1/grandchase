...
KLuaManager::KLuaManager( lua_State* pLuaState )
{
    ...
    for( int i = 1 ; i <= lua_gettop( m_pkLuaState ) ; ++i ) {
        if( lua_type( m_pkLuaState, i ) != LUA_TTABLE )
            continue;
...
*/
HRESULT KLuaManager::DoString(const char* pszStr, int iSize)
{
    if (luaL_dostring(m_pkLuaState, pszStr, iSize) == 0)
    {
        return S_OK;
    }
    else
...
{
    if (luaL_dofile(m_pkLuaState, pszFileName) == 0)
    {
        return S_OK;
    }
    else
...
HRESULT KLuaManager::Get(const char* pszName)
{
    if ( m_iTableDepth > 0 )
    {
        if( lua_istable( m_pkLuaState, -1 ) == false )
            return E_FAIL;
    ...
    {
        if( lua_gettop( m_pkLuaState ) == 0 ) {
            lua_getglobal(m_pkLuaState, pszName);
        }
        else {
            if( lua_type( m_pkLuaState, -1 ) != LUA_TTABLE ) {
                return E_FAIL;
            }
...
{
    if (m_iTableDepth <= 0) return E_FAIL;
...
/** (table 내에서) iIndex번째 변수의 값을 가져온다.
@param iIndex [in] 변수의 index (1부터 시작함)
@param i64Value [out] 숫자 변수의 값 (실패시는 변화 없음)
@param bLogOutput [in] false로 설정하면 log message를 남기지 않음; true가
default임.
@return S_OK for success, E_FAIL for failure (해당 변수가 없거나, 숫자 변수가
아닌 경우).
*/
HRESULT KLuaManager::GetBigINT(int iIndex, __int64& i64Value, bool bLogOutput)
{
    if (SUCCEEDED(Get(iIndex)) && lua_isnumber(m_pkLuaState, -1) != 0)
    { 
        i64Value = lua_tointeger(m_pkLuaState, -1);
        lua_pop(m_pkLuaState, 1);
        return S_OK;
    }
    else
...
@param i64Value [out] 문자열 변수의 값 (실패시는 변화 없음)
@param bLogOutput [in] false로 설정하면 log message를 남기지 않음; true가
default임.
@return S_OK for success, E_FAIL for failure (해당 변수가 없거나, 문자열 변수가
아닌 경우).
*/
HRESULT KLuaManager::GetBigINT(const char* pszName, __int64& i64Value, bool bLogOutput)
{
    if (SUCCEEDED(Get(pszName)) && lua_isnumber(m_pkLuaState, -1) != 0)
    {
        i64Value = lua_tointeger(m_pkLuaState, -1);
...
HRESULT KLuaManager::GetValue( const char* pszName, UCHAR& ucValue, bool bLogOutput )
{
    GET_BY_NAME(ucValue, UCHAR);
}

HRESULT KLuaManager::GetValue( int iIndex, UCHAR& ucValue, bool bLogOutput )
{
    GET_BY_NUM(ucValue, UCHAR);
}
