...
static int tostring_string(lua_State *L)
{
    static char temp[2048];
    const std::string* pstr = (std::string*)lua_topointer( L, 1 );
    strcpy_s( temp, 2048, pstr->c_str() );
...
/*---------------------------------------------------------------------------*/ 
static int eq_string(lua_State *L)
{
    lua_pushboolean(L, strcmp( ((const std::string*)lua_topointer(L, 1))->c_str(), ((const std::string*)lua_topointer(L, 2) )->c_str()) );
    return 1;
}

/*---------------------------------------------------------------------------*/ 
static int lt_string(lua_State *L)
{
    lua_pushboolean(L, strcmp( ((const std::string*)lua_topointer(L, 1))->c_str(), ((const std::string*)lua_topointer(L, 2) )->c_str()) );
    return 1;
}
...
/*---------------------------------------------------------------------------*/ 
static int le_string(lua_State *L)
{
    lua_pushboolean(L, strcmp( ((const std::string*)lua_topointer(L, 1))->c_str(), ((const std::string*)lua_topointer(L, 2) )->c_str()) );
    return 1;
}
...
static int set_string(lua_State *L ) {
    std::string* str = (std::string*)lua_topointer(L,1);
    const char* arg = lua_tostring(L, 2);
    (*str) = arg;

    static char temp[2048];
    strcpy_s( temp, 2048, arg );
    lua_pushstring( L, temp );
    return 1;
}

static int get_string(lua_State *L ) {
    return tostring_string( L );
}

void lua_tinker::init_string( lua_State *L )
{
    const char* name = "__stdstring";
    lua_pushstring(L, name);
    lua_newtable(L);
    ...
    lua_pushstring(L, "__tostring");
    lua_pushcclosure(L, tostring_string, 0);
    lua_rawset(L, -3);

    lua_pushstring(L, "__eq");
    lua_pushcclosure(L, eq_string, 0);
    lua_rawset(L, -3);	

    lua_pushstring(L, "__lt");
    lua_pushcclosure(L, lt_string, 0);
    lua_rawset(L, -3);	

    lua_pushstring(L, "__le");
    lua_pushcclosure(L, le_string, 0);
    lua_rawset(L, -3);	

    lua_pushstring(L, "get" );
    lua_pushcclosure(L, get_string, 0);
    lua_rawset(L, -3);	

    lua_pushstring(L, "set" );
    lua_pushcclosure(L, set_string, 0);
    lua_rawset(L, -3);	
...
__int64 lua_tinker::read(lua_State *L, int index)
{
	if(lua_isnumber(L,index))
        ...
        __int64 *pInt64 = (__int64*)lua_touserdata(L, index);
        if(pInt64 == NULL) return 0;
		return *pInt64;
    }
}
template<>
unsigned __int64 lua_tinker::read(lua_State *L, int index)
{
	if(lua_isnumber(L,index))
        ...
        unsigned __int64 *pUInt64 = (unsigned __int64*)lua_touserdata(L, index);
        if(pUInt64 == NULL) return 0;
        return *pUInt64;
    }
}
template<>
