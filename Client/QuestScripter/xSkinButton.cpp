...
void CxSkinButton::SetSkin(CString normal, CString down, CString over, CString disabled, CString focus, CString mask,
	short drawmode, short border, short margin)
{
	if ((HBITMAP)(m_bNormal)) m_bNormal.Detach();
	if ((HBITMAP)(m_bNormal)) m_bDown.Detach();
	if ((HBITMAP)(m_bNormal)) m_bOver.Detach();
	if ((HBITMAP)(m_bNormal)) m_bDisabled.Detach();
	if ((HBITMAP)(m_bNormal)) m_bMask.Detach();
	if ((HBITMAP)(m_bNormal)) m_bFocus.Detach();

	if (normal.GetLength()) SetBitmapFromFile(normal, &m_bNormal);
	if (down.GetLength()) SetBitmapFromFile(down, &m_bDown);
	if (over.GetLength()) SetBitmapFromFile(over, &m_bOver);
	if (focus.GetLength()) SetBitmapFromFile(focus, &m_bFocus);

	if (disabled.GetLength()) SetBitmapFromFile(disabled, &m_bDisabled);
	else if (normal.GetLength()) SetBitmapFromFile(normal, &m_bNormal);
	...
	if (mask.GetLength()) {
		SetBitmapFromFile(mask, &m_bMask);
		if (hClipRgn) DeleteObject(hClipRgn);
		hClipRgn = CreateRgnFromBitmap(m_bMask, RGB(255, 255, 255));
		if (hClipRgn) {
			SetWindowRgn(hClipRgn, TRUE);
...
void CxSkinButton::SetBitmapFromFile(CString strFile, CBitmap *bitmap)
{
	HBITMAP hBitmap = (HBITMAP)::LoadImage(AfxGetApp()->m_hInstance, strFile, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);

	if (hBitmap == NULL)
		return; // bitmap Load Fail

	if ((HBITMAP)(*bitmap)) //비트맵 핸들이 존재시 삭제
		bitmap->Detach();
...
void CxSkinButton::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if (GetParent())
	{
		GetParent()->PostMessage(WM_CLICK_TOOL_BUTTON, 0, (LPARAM)this);
	}
}
...
void CxSkinButton::GetTooltipText(CString *strText)
{
	m_tooltip.GetText(*strText, this);
}