	...
	if (m_iMode & _MODE_FILESYSTEM_ || m_iMode & _MODE_FS_START_DIR_)
	{
		if (!m_CursorMode)
		{
			...
			else
			{
				if (len > 2 && pos == len)
				{
					std::string strTemp = GCUTIL_STR::GCStrWideToChar(m_EditText);
...
int CACEdit::InsertString(int index, LPCTSTR lpszString)
{
	if (m_iType == _COMBOBOX_)
	{
		return ((CComboBox *)this)->InsertString(index, lpszString);
	}
	return CB_ERR;
}
...
// Jaeho.add
int CACEdit::FindStringExact(int nStartAfter, LPCTSTR lpszString)
{
	if (m_iType == _COMBOBOX_)
	{
		return ((CComboBox *)this)->FindStringExact(nStartAfter, lpszString);
	}
	return CB_ERR;
}
...
// Jaeho.Add
int CACEdit::SetCurSel(int iSel)
{
	if (m_iType == _COMBOBOX_)
	{
		return ((CComboBox *)this)->SetCurSel(iSel);
	}
	return CB_ERR;
}
