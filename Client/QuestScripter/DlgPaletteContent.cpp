...
IMPLEMENT_DYNAMIC(CDlgPaletteContent, CDialog)
CDlgPaletteContent::CDlgPaletteContent(CWnd* pParent /*=NULL*/)
: CDialog(CDlgPaletteContent::IDD, pParent)
{
    m_enType = OBJECTTYPE_EACTION_PLAYER;
}

CDlgPaletteContent::~CDlgPaletteContent()
{
...
void CDlgPaletteContent::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CDlgPaletteContent, CDialog)
...
// CDlgPaletteContent 메시지 처리기입니다.

void CDlgPaletteContent::OnBnClickedOk()
{
    // TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
    //OnOK();
}

void CDlgPaletteContent::OnBnClickedCancel()
{
    // TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
    //OnCancel();
}

void CDlgPaletteContent::CreateButtonWindow(void)
{
    ...
    int iSBCnt = 0;
    std::map<CString, CString>::iterator itItem = SiCObjectDataManager()->GetObjImgItem(m_enType).begin();
    for(;itItem != SiCObjectDataManager()->GetObjImgItem(m_enType).end();itItem++)
    {
        CxSkinButton *pButton;
        pButton = new CxSkinButton;

        pButton->Create(L"", WS_CHILD | WS_VISIBLE, rtButton, this, WM_USER+iSBCnt);

        pButton->SetToolTipText(itItem->first);
        CString strFileName = GCUTIL_STR::GCStrCharToWide( g_Path );
        strFileName += itItem->second;
...
int CDlgPaletteContent::ReSizeButton(int iStartVerLine)
{
    const int ibtWidth = 30;
    const int ibtHeight = 30;
...
void CDlgPaletteContent::OnSize(UINT nType, int cx, int cy)
{
...
void CDlgPaletteContent::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
...
LRESULT CDlgPaletteContent::OnClickToolButton(WPARAM wParam, LPARAM lParam)
{
    if( GetParent() )
...
void CDlgPaletteContent::OnCbnSelchangeComboId()
{
    // TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
...
BOOL CDlgPaletteContent::OnInitDialog()
{
...
void CDlgPaletteContent::Refresh(void)
{
    int cnt = 0;
    m_cbToolPalette.ResetContent();    
    std::map<CString, CString>::iterator itMon = SiCObjectDataManager()->GetObjImgItem(m_enType).begin();
    for(;itMon != SiCObjectDataManager()->GetObjImgItem(m_enType).end();itMon++, cnt++)
    {
        m_cbToolPalette.AddSearchString(itMon->first);
        m_cbToolPalette.InsertString(cnt, itMon->first);
    }
...
//void CDlgPaletteContent::SetMonImage(std::vector<TPObject> &vtTPObject)
//{
//    m_vtTPObject.clear();
