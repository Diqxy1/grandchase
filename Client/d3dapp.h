...
struct D3DDeviceInfo
{
    ...
    DWORD        dwNumModes;
    KSafeArray<D3DModeInfo,150> modes;
...
struct D3DAdapterInfo
{
    ...
    DWORD          dwNumDevices;
    KSafeArray<D3DDeviceInfo,5> devices;
...
struct GCFrameCheck
{
    ...
    {
        dwMaxFrame          = 0;
        dwMinFrame          = 55;       // 최소 프레임 수가 0이면 더 줄어들 프레임이 없으므로.
        dwSumFrames         = 0;
        dwSumCount          = 0;
    }
...
public:
    // Internal variables for the state of the appy
    
    bool              IsPrimaryBuffer;
    
    bool			  m_IsFirstStart;
	KSafeArray<D3DAdapterInfo,10> m_Adapters;
    ...
    void SetWindowMode(BOOL bWindowed_ );
    ...
    int               m_FPS;               // Instanteous frame rate
    UINT              m_uiPacketPerSecond; // Packet Count Per Second
    TCHAR             m_strDeviceStats[90];// String to hold D3D device stats
    TCHAR             m_strFrameStats[90]; // String to hold frame stats
	BOOL			  m_bKeepFPS60;        // Keep Rendering in FPS 55
...
public:
    ...
    void StartFarmeCheck(void);
    void CheckFrame(void);
    void EndFrameCheck(void);

protected:
    bool            m_bFrameCheck;
    GCFrameCheck    m_FrameCheck;
};
