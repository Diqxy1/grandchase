#include "stdafx.h"
// KGCPlayerLire2.cpp: implementation of the KGCPlayerLire2 class.
//
//////////////////////////////////////////////////////////////////////
...
int KGCPlayerLire2::OnDash(PLAYER* pPlayer, bool bKeyProc)
{
	if(bKeyProc)
	{
...
int KGCPlayerLire2::OnSpecial1(PLAYER* pPlayer, bool bKeyProc)
{
	if( bKeyProc )
	{
		if( g_MyD3D->MyCtrl->k_Up )
		{
			...
			{
				//Frame은 그대로 유지합니다
				pPlayer->SetPlayerState(MID_LIRE2_SPECIAL1_UP);				
			}			
		}
		return 0;
	}
...
int KGCPlayerLire2::OnComboAtk(PLAYER* pPlayer, bool bKeyProc)
{
	if(bKeyProc)
	{
		if( pPlayer->uiMotion == MID_LIRE2_ATK1_5 ||
			pPlayer->uiMotion == MID_LIRE2_ATK2_5 ||
			pPlayer->uiMotion == MID_LIRE2_ATK3_5 )
			return 0;
		if( g_MyD3D->MyCtrl->k_Fresh_Punch )
		{
		...
		//활 못쏘게함.
		//if( pPlayer->Frame == 4 && diks[DIK_Z] && ( pPlayer->Now_Motion == MID_LIRE2_ATK1_1 ||pPlayer->Now_Motion == MID_LIRE2_ATK2_1 ||
		//////////////////////////////////////////////////////////////////////////
		// 2006. 11. 27 권상구 : pPlayer->Frame 가 4였는데 리플레이에서 석궁을 미친듯이 쏴서 3으로 수정
		////////////////////////////////////////////////////////////////////////// 
		if( pPlayer->cFrame == 3 && diks[g_MyD3D->MyCtrl->m_keyAttack] && ( pPlayer->uiMotion == MID_LIRE2_ATK1_1 ||pPlayer->uiMotion == MID_LIRE2_ATK2_1 ||
			 pPlayer->uiMotion == MID_LIRE2_ATK3_1 ) )
		{
		...
		{		
			if( ( pPlayer->cFrame == 34 && pPlayer->uiMotion % 5 == 0 ) ||
				( pPlayer->cFrame == 24 && (pPlayer->uiMotion % 5 == 1 || pPlayer->uiMotion % 5 == 2 || pPlayer->uiMotion % 5 == 3 ) )  )
			{
				int Plus = pPlayer->uiMotion % 5 + 1;                 

				//if( diks[DIK_UP] )
				if( diks[g_MyD3D->MyCtrl->m_keyUp] )
					pPlayer->Reserve_Motion = (MID_LIRE2_ATK2_1+Plus);
				//else if( diks[DIK_DOWN] ) 
				else if( diks[g_MyD3D->MyCtrl->m_keyDown] ) 
					pPlayer->Reserve_Motion = (MID_LIRE2_ATK3_1+Plus);
				else if( g_MyD3D->MyCtrl->k_Right || g_MyD3D->MyCtrl->k_Left )
				{                                
					pPlayer->Reserve_Motion = MID_LIRE2_ATK1_1+Plus;
				}
				else
				{
					if( pPlayer->uiMotion >= (UINT)(MID_LIRE2_ATK1_1) && pPlayer->uiMotion < (UINT)(MID_LIRE2_ATK1_5) )
						pPlayer->Reserve_Motion = MID_LIRE2_ATK1_1+Plus;
					else if( pPlayer->uiMotion >= (UINT)(MID_LIRE2_ATK2_1) && pPlayer->uiMotion < (UINT)(MID_LIRE2_ATK2_5) )
						pPlayer->Reserve_Motion = MID_LIRE2_ATK2_1+Plus;
					else if( pPlayer->uiMotion >= (UINT)(MID_LIRE2_ATK3_1) && pPlayer->uiMotion < (UINT)(MID_LIRE2_ATK3_5) ) 
						pPlayer->Reserve_Motion = MID_LIRE2_ATK3_1+Plus;
				}				
				g_MyD3D->MyCtrl->k_Up = false;    
				g_MyD3D->MyCtrl->k_Down = false;

				pPlayer->Reserve_IsRight = pPlayer->bIsRight;
...
int KGCPlayerLire2::OnJumpReady(PLAYER* pPlayer, bool bKeyProc)
{
	if( bKeyProc )
	{
		//if( !diks[DIK_UP] || pPlayer->Frame > 8 )
		if( !diks[g_MyD3D->MyCtrl->m_keyUp] || pPlayer->cFrame > 8 )
		{
			pPlayer->y_Speed		= (float)pPlayer->cFrame * 0.003f + ( pPlayer->GetCurFormTemplate().fJumpYSpeed * 0.5f );
			pPlayer->SetPlayerState(pPlayer->GetCurFormTemplate().JUMP);
			pPlayer->cFrame		= 0;        
			pPlayer->IsContact	= false;
		}
		if( pPlayer->Item_Remain_Time[GC_GAME_ITEM_WING] ) 
...
int KGCPlayerLire2::OnJump(PLAYER* pPlayer, bool bKeyProc)
{
	if(bKeyProc)
	{
		//천천히 떨어지게 함
		if( pPlayer->y_Speed > 0 && diks[g_MyD3D->MyCtrl->m_keyDown] && g_MyD3D->MyCtrl->k_Fresh_Punch )
        {
			pPlayer->SetPlayerState(MID_LIRE2_JUMPPING_DOWN_ARROW_ATK);
			pPlayer->cFrame = 0;
			return 0;
        }
		...
		KGCPlayerCommon::OnJump(pPlayer,bKeyProc);		
		return 0;
	}

	KGCPlayerCommon::OnJump(pPlayer,false);
	return 0;
}

int KGCPlayerLire2::OnJumpAtk(PLAYER* pPlayer, bool bKeyProc)
{
	if(bKeyProc)
	{
		if(g_MyD3D->MyCtrl->k_Fresh_Punch)
    ...
    if ( pPlayer->vPos.y >= -0.23f )
    {
	if(pPlayer->y_Speed < -0.01f)
		pPlayer->y_Speed = -0.01f;
    }
//////////////////////////////////////////////////////////////////////////////////
	pPlayer->IsContact = false;
...
int KGCPlayerLire2::OnDamageAmplifier(PLAYER* pPlayer, bool bKeyProc)
{
...
int KGCPlayerLire2::OnJumppingDownArrowAtk(PLAYER* pPlayer, bool bKeyProc)
{
	if(bKeyProc)
	{
		if(g_MyD3D->MyCtrl->k_Fresh_Punch)
...
int KGCPlayerLire2::OnWait( PLAYER* pPlayer, bool bKeyProc )
{
	if(bKeyProc)
	{
