                                        ...
                                        DWORD dwNumBuffers )
{
    ...
    pWaveFile->OpenFromMemory( pbData,ulDataSize, WAVEFILE_READ );
...
unsigned long CSound::GetFreeBufferNum()
{
	DWORD i;
    for( i=0; i<m_dwNumBuffers; i++ )
    {
...
HRESULT CWaveFile::OpenFromMemory( BYTE* data, ULONG size, DWORD dwFlags )
{
	HRESULT hr;
...
HRESULT CWaveFile::ReadMMIO()
{
    ...
    if( (m_ckRiff.ckid != FOURCC_RIFF) ||
        (m_ckRiff.fccType != mmioFOURCC('D', 'A', 'T', 'T') ) )
