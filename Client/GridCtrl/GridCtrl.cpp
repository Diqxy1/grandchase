...
{        
    ...
    if (pSource) 
    {   
		m_FirstDragOverCell.row = GetSelectedCellRange().GetTopLeft().row;
		m_FirstDragOverCell.col = GetSelectedCellRange().GetTopLeft().col;
...
void CGridCtrl::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
    ...
    else if (m_MouseMode == MOUSE_NOTHING)
    {
        ...
        {
            OnEditCell(m_LeftClickDownCell.row, m_LeftClickDownCell.col, VK_LBUTTON);
        }
        else if (m_bListMode)
        {
            CCellID cell = GetCellFromPt(point);
