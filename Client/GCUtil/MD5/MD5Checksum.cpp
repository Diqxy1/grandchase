...
std::wstring CMD5Checksum::GetMD5( char* chFilePath_ )
{
	CMD5Checksum MD5Checksum;
	FILE *fp = NULL;
	unsigned char uData[ MAX_FILE_MD5_BUFFER ];
	size_t ln;

	if ( ( fp = fopen( chFilePath_, "rb" ) ) == NULL )
		return L"";

	while(!feof(fp))
	{
		ln = fread(uData, 1, sizeof(uData), fp);
		MD5Checksum.Update( uData, (ULONG)ln );	
	}	
...
std::wstring CMD5Checksum::GetMD5(BYTE* pBuf, UINT nLength)
{
	//entry invariants
...
std::wstring CMD5Checksum::Final()
{
	...
	for ( int i=0; i < nMD5Size; i++) 
	{
        WCHAR szPart[4];
		if (lpszMD5[i] == 0) {
            ::lstrcpyW( szPart, L"00" );
		}
		else if (lpszMD5[i] <= 15) 	{
            ::wsprintfW( szPart, L"0%x",lpszMD5[i] );
		}
		else {
            ::wsprintfW( szPart, L"%x",lpszMD5[i] );
		}

        ASSERT( ::lstrlenW( szPart ) == 2 );
        strMD5 += szPart;
	}
