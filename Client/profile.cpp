...
void CPROFILE::Begin(char* name)
{
	...
	StringCchCopyA(m_samples[i].szName, 256, name);
...
void CPROFILE::End(char* name)
{
	...
	while (i < NUM_PROFILE_SAMPLES && m_samples[i].bValid == true)
	{
		if (strcmp(m_samples[i].szName, name) == 0)	//Found the sample
		{
			...
			if (parent >= 0)
			{
				if (fEndTime > m_samples[i].fStartTime)
				{
...
void CPROFILE::DumpOutputToFile()
{
	...
	if (!m_pLogFile)
	{
		...
		std::string strFileName = "./log/frame_profile_";
		strFileName += GCUTIL_STR::GCStrWideToChar((LPCTSTR)(CTime::GetCurrentTime().Format(_T("%Y%m%d_%H%M%S"))));
		strFileName += ".txt";

		m_pLogFile = fopen(strFileName.c_str(), "wb");

		if (m_pLogFile)
		{
			fprintf(m_pLogFile, "\t\tAve\tMin\tMax\tSam\tPer\t#\tProfile Name\n");
			fprintf(m_pLogFile, "----------------------------------------------------------------------------\n");
		}
	}
	...
	std::string strCurTime = GCUTIL_STR::GCStrWideToChar((LPCTSTR)(CTime::GetCurrentTime().Format(KNC_TIME_FORMAT)));
	...
	while (i < NUM_PROFILE_SAMPLES && m_samples[i].bValid == true)
	{
		...
		{
			char szDump[256] = "";
			StringCchPrintfA(szDump,
				256,
				"%s, ProfileEnd() called without a ProfileBegin()",
				m_samples[i].szName);
			ASSERT(!szDump);
		}
		else if (m_samples[i].iOpenProfiles > 0)
		{
			...
			StringCchPrintfA(szDump,
				256,
				"%s, ProfileBegin() called without a ProfileEnd()",
				m_samples[i].szName);
			ASSERT(!szDump);
		}

		if (m_samples[i].fAccumulator < m_samples[i].fChildrenSampleTime)
		{
			sampleTime = fDxTIME;
		}
		else
		...
		sampleTime = (float)m_samples[i].fAccumulator;
		...
		//Format the data
		StringCchPrintfA(ave, 16, "%3.1f", aveTime);
		StringCchPrintfA(min, 16, "%3.1f", minTime);
		StringCchPrintfA(max, 16, "%3.1f", maxTime);
		StringCchPrintfA(sam, 16, "%3.1f", sampleTime);
		StringCchPrintfA(per, 16, "%3.1f", percentTime);
		StringCchPrintfA(num, 16, "%3d", m_samples[i].iProfileInstances);

		StringCchCopyA(indentedName, 256, m_samples[i].szName);
		for (indent = 0; indent < m_samples[i].iNumParents; indent++)
		{
			StringCchPrintfA(name, 256, "   %s", indentedName);
			StringCchCopyA(indentedName, 256, name);
		}

		fprintf(m_pLogFile, "%d\t%s\t", iFrame, strCurTime.c_str());

		fprintf(m_pLogFile, "%5s\t%5s\t%5s\t%5s\t%5s\t%3s\t%s\n", ave, min, max, sam, per, num, indentedName);
...
void CPROFILE::StoreInHistory(char* name, float percent)
{
	...
	{
		StringCchCopyA(m_history[i].szName, 256, name);
