...
const std::wstring& KGCMissionString::GetString( int iID )
{
    std::map< int, std::wstring >::iterator mit = m_mapMissionString.find( iID );
    if( mit == m_mapMissionString.end() )
    {
        assert( !"mit == m_mapMissionString.end()" );
        static std::wstring strEmpty(L"");
        return strEmpty; // literal..
    }
    return mit->second;
}
