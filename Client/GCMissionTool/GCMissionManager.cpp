...
bool KGCMissionManager::LoadMissionFile( std::string strMissionScriptFile , KLuaManager* pkLuaMgr )
{
    ...
    if( SUCCEEDED( luaMgr.BeginTable( "Condition" ) ) )
    {
        for( int i = 1 ; ; ++i )
        {
            LUA_BEGIN_TABLE( i, break )
            {
                ...
                {
                    OKBOXW( L"Condition Table Error : "<<i , L"Error");
                    luaMgr.EndTable();
                    break;
                }
				...
				LUA_GET_VALUE_DEF( "IsSolo", sCondition.m_bSolo, false );
    ...
    if( SUCCEEDED( luaMgr.BeginTable( "Submission" ) ) )
    {
        for( int i = 1 ; ; ++i )
        {
            LUA_BEGIN_TABLE( i, break )
            {
                ...
                {
                    OKBOXW( L"SubMission Table Error : "<<i , L"Error" );
                    luaMgr.EndTable();
                    break;
                }
                ...
                {
                    OKBOXW( L"No Condition Error : "<<sSubMissionInfo.m_iSubMissionID , L"Error" );
                    luaMgr.EndTable();
                    break;
                }
                ...
                {
                    OKBOXW( L"No Action Error : "<<sSubMissionInfo.m_iSubMissionID, L"Error" );
                    luaMgr.EndTable();
                    break;
                }
                ...
                if( SUCCEEDED( luaMgr.BeginTable( "Completion" ) ) )
                {
                    ...
                    {
                        OKBOXW( L"No Completion Count Error : "<<sSubMissionInfo.m_iSubMissionID, L"Error" );
                    }
                    luaMgr.EndTable();
                }
                else
                {
                    OKBOXW( L"No Completion Error : "<<sSubMissionInfo.m_iSubMissionID, L"Error" );
                    luaMgr.EndTable();
                    break;
                }
    ...
    {
        OKBOXW( L"Submission read Failed!!!" , L"Error");
        return false;
    }
            ...
            LUA_BEGIN_TABLE( i, break )
            {
                ...
                if( SUCCEEDED( luaMgr.BeginTable( "RewardItem" ) ) )
                {
                    for( int iRICount = 1; ; ++iRICount )
                    {
                        ...
                        sMissionInfo.m_vecRewardItem.push_back( std::make_pair( iItemID, iCount ) );
                        luaMgr.EndTable();
                    }
                    luaMgr.EndTable();
                }
                ...
                if( SUCCEEDED( luaMgr.BeginTable( "SelectItem" ) ) )
                {
                    for( int iSICount = 1; ; ++iSICount )
                    {
                        ...
                        sMissionInfo.m_vecSelectItem.push_back( std::make_pair( iItemID, iCount ) );
                        luaMgr.EndTable();
                    }
                    luaMgr.EndTable();
                }
...
{
    FILE* pFile = fopen( strMissionScriptFile.c_str(), "wc" );

    if( pFile == NULL )
    {
        OKBOXW( L"File Open Error", L"Error" );
        return false;
    }
...
int KGCMissionManager::CheckCompleteMission( DWORD dwMissionID, std::vector< std::pair< int, int > > vecCompletion )
{
    ...
    {
        if( m_mapSubMission.count( vecCompletion[i].first ) == 0 )
            return GC_MM_SUBMISSION_NOT_EXIST;

        if( m_mapSubMission[ vecCompletion[i].first].IsComplete( vecCompletion[i].second ) == false)
...
int KGCMissionManager::IsAvailableRegister( DWORD dwMissionID, int iChar, int iLevel, int iPromotion )
{
...
bool KGCMissionManager::CheckSubMissionCondition( int iSubMissionID, const SConditionInfo& sInfo )
{
	std::map< int, KGCSubMission >::iterator mit;
