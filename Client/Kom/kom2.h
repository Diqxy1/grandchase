...
public:
	...
	KSubfile(Komfile * parent, std::ifstream & stream, int headersize); // 구 버전 콤에서 읽는 함수
	KSubfile(Komfile * parent, xmlNode * node, int * offset); // URL 신버전
	...
	bool WriteDecompressedFileOnFolder(const std::string fileName, const std::string folderName);
	...
	//std::string GetParentName(){return filename;}
...
protected:
	void DecryptHeader(unsigned int key, char * data, int length);
