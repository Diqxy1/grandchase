...
long DisplayHTMLPage(HWND hwnd, const char* webPageName, const char* strPostString_, const wchar_t* wstrTargetFrameName_)
{
	IWebBrowser2* webBrowser2;
	VARIANT			myURL;
	IOleObject* browserObject;
	...
	if (!browserObject->lpVtbl->QueryInterface(browserObject, &IID_IWebBrowser2, (void**)& webBrowser2))
	{
		...
		// Call the Navigate2() function to actually display the page.
		if (strlen(strPostString_) == 0) {
			if (L"" != wstrTargetFrameName_) {
				OLECHAR ctarget[MAX_PATH];
				VARIANT vaTarget;

				wcscpy(ctarget, wstrTargetFrameName_);
				vaTarget.vt = VT_BSTR;
				vaTarget.bstrVal = SysAllocString(ctarget);

				webBrowser2->lpVtbl->Navigate2(webBrowser2, &myURL, 0, &vaTarget, 0, 0);
			}
			else {
				webBrowser2->lpVtbl->Navigate2(webBrowser2, &myURL, 0, 0, 0, 0);
			}
		...
		else
		{
			...
			hr = webBrowser2->lpVtbl->Navigate2(webBrowser2, &myURL, 0, &vaTarget, &vaPost, &vaPostHeader);
			VariantClear(&vaPost);
			VariantClear(&vaTarget);
