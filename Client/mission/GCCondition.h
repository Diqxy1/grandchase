...
struct SConditionInfo
{
    ...
    std::pair< int, int >   m_pairMonLevelRange;
    int     m_iType;
    int     m_iValue;
    std::pair< int, int >   m_pairUseCount;
    SConditionInfo()
    {
        m_dwID                      = -1;
        m_iMonsterID                = -1;
        ...
        m_pairMonLevelRange         = std::make_pair( -1, -1 );
        m_iType                     = -1;
        m_iValue                    = -1;
        m_pairUseCount              = std::make_pair( -1, -1 );
    }
    ...
    bool operator==( const SConditionInfo& arg )
    {
        ...
        (m_pairMonLevelRange         == arg.m_pairMonLevelRange )&&
        (m_iType                     == arg.m_iType             )&&
        (m_iValue                    == arg.m_iValue            )&&
        (m_pairUseCount              == arg.m_pairUseCount      );
    }
};
class KGCCondition
{
...
#define GC_CONDITION_SATISFIED( value, initValue, argValue ) \
	...
	bool IsSatisfied( const SConditionInfo& arg )
	{
        ...
        GC_CONDITION_SATISFIED( m_sCondition.m_pairMonLevelRange,   std::make_pair( -1, -1 ), arg.m_pairMonLevelRange );
        GC_CONDITION_SATISFIED( m_sCondition.m_iType,		-1, arg.m_iType );
        GC_CONDITION_SATISFIED( m_sCondition.m_iValue,		-1, arg.m_iValue );
        GC_CONDITION_SATISFIED( m_sCondition.m_pairUseCount,        std::make_pair( -1, -1 ), arg.m_pairUseCount );
		if( m_sCondition.m_bWin != arg.m_bWin )
			return false;
		if( m_sCondition.m_nSolo != arg.m_nSolo )
			return false;
		return true;
