...
class KGCCTMissionManager
{
    DeclareSingleton(KGCCTMissionManager);
public:
    KGCCTMissionManager(void);
    ~KGCCTMissionManager(void);
...
public:
    ...
    bool    IsMissionExist( DWORD dwMissionID ) {
        return ( ( m_mapMissionList.find( dwMissionID ) == m_mapMissionList.end() ) ? false : true );
    }
    bool LoadMissionStringFromKOM( std::string strFileName );
};
DefSingletonInline(KGCCTMissionManager);
