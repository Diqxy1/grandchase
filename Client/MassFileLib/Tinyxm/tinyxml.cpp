...
bool TiXmlDocument::LoadFile( const char* filename )
{
	...
	value = filename;
    FILE* file = NULL;
    errno_t err = fopen_s( &file, value.c_str (), "r" );
...
void TiXmlDeclaration::Print( FILE* cfile, int /*depth*/ ) const
{
	...
	if ( !version.empty() )
		fprintf_s (cfile, "version=\"%s\" ", version.c_str ());
	if ( !encoding.empty() )
		fprintf_s (cfile, "encoding=\"%s\" ", encoding.c_str ());
	if ( !standalone.empty() )
		fprintf_s (cfile, "standalone=\"%s\" ", standalone.c_str ());
	fprintf_s (cfile, "?>");
}
