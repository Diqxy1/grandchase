...
KGCStateCashShop::KGCStateCashShop(void)
{
}
...
bool KGCStateCashShop::OnInitBeforeStateChange(GAME_STATE eNowState_ )
{
	g_MyD3D->m_pStateMachine->Pushfront_SavedState( eNowState_ );
	return true;
}

bool KGCStateCashShop::FrameMove(float fElapsedTime)
{
...
bool KGCStateCashShop::OnInit()
{
	...
	g_pkUIScene->m_pkCashShopScene->EnterScene();

	//g_pkUIScene->m_pkCashShopScene->SetButtonLock( BTN_LOBBY , !g_MyD3D->m_pStateMachine->IsJoinedChannel() );

    g_RenderManager->SetOutlineWidth( OUT_LINE_WIDTH_IN_SHOP );
	return true;
}

bool KGCStateCashShop::OnDestroyBeforeStateChange()
{
	return true;
}

void KGCStateCashShop::OnSetupFSM()
{
...
void KGCStateCashShop::OnDestroy()
{
    g_pkUIScene->m_pkCashShopScene->LeaveScene();
