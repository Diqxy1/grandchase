...
#define PIXELTOTEXEL_X_LEFT(p) ( (float)((p) + 0.5f ) / (float) m_pTexture->GetWidth() )
#define PIXELTOTEXEL_Y_TOP(p) ( (float)((p) + 0.5f ) / (float) m_pTexture->GetHeight() )
#define PIXELTOTEXEL_X_RIGHT(p) ( (float)((p) - 0.5f ) / (float) m_pTexture->GetWidth() )
#define PIXELTOTEXEL_Y_BOTTOM(p) ( (float)((p) - 0.5f ) / (float) m_pTexture->GetHeight() )
...
KLayerPart<T>::KLayerPart(void) : m_strPartName(""), m_pTexture(NULL), m_fLayerX(0), m_fLayerY(0),
                               ...
                               m_bShow(true),
                               //m_bUseEffect(false), 
                               m_fZoffset(0), m_bPlayerIsBack(false), m_bScroolX( false ), m_bScroolY( false ), m_fCMRadioX( 0.0f ), m_fCMRadioY( 0.0f )
{
    for( int i = 0 ; i < (int)m_fUV.size() ; ++i )
    {
        m_fUV[i] = 0.0f;
    }
}
...
void KLayerPart<T>::SetTexel()
{
	...
	m_fUV[0] = PIXELTOTEXEL_X_LEFT(m_iTexX);
	m_fUV[1] = PIXELTOTEXEL_Y_TOP(m_iTexY);
	m_fUV[2] = PIXELTOTEXEL_X_RIGHT(m_iTexX + m_iTexWidth);
	m_fUV[3] = PIXELTOTEXEL_Y_BOTTOM(m_iTexY + m_iTexHeight);
}
