...
IMPLEMENT_CLASSNAME(KGCGPShopScene);
IMPLEMENT_WND_FACTORY(KGCGPShopScene);
IMPLEMENT_WND_FACTORY_NAME(KGCGPShopScene, "gc_gp_shop_scene");

KGCGPShopScene::KGCGPShopScene(void)
    ...
    , m_iCurTabCategory(TAB_ARMOR)
{
    ...
    LINK_CONTROL("exit_btn", m_pkBtnExit);
...
void KGCGPShopScene::OnCreate()
{
    ...
    g_pkUIMgr->ConnectGlobalEventProcedure(KGCUIScene::D3DWE_CHANGE_CHAR_INFO, this, &KGCGPShopScene::OnChangeItemInfo);


#if defined(BUY_CASH_ITEM_WITH_POINT)

#if defined(NATION_KOREA)
    m_pkBtnPointItemToggle->InitState(false, false, NULL);
    m_pkBtnCashItemToggle->InitState(false, false, NULL);
#else
...
void KGCGPShopScene::OnEnterScene()
{
...
void KGCGPShopScene::InitPointshopTap()
{
#if defined(BUY_CASH_ITEM_WITH_POINT)
...
void KGCGPShopScene::OnLeaveScene()
{
    ResetEquipItems();
...
void KGCGPShopScene::OnDestroy()
{
    g_MyD3D->m_TempPlayer.ReleasePlayerResource();
}
void KGCGPShopScene::OnCreateComplete()
{
}

void KGCGPShopScene::ActionPerformed(const KActionEvent& event)
{
    ...
    {
        if (event.m_pWnd == m_apkShopTab[TAB_ARMOR])
            m_apkShopTab[TAB_ARMOR]->SetToolTip(g_pkStrLoader->GetString(STR_ID_TOOLTIP_GPSHOP_PURCHASE_GUIDE));
        if (event.m_pWnd == m_apkShopTab[TAB_WEAPON])
            m_apkShopTab[TAB_WEAPON]->SetToolTip(g_pkStrLoader->GetString(STR_ID_TOOLTIP_GPSHOP_PURCHASE_GUIDE));
        if (event.m_pWnd == m_apkShopTab[TAB_SET])
            m_apkShopTab[TAB_SET]->SetToolTip(g_pkStrLoader->GetString(STR_ID_TOOLTIP_GPSHOP_PURCHASE_GUIDE));
    }
...
void KGCGPShopScene::ResetEquipItems()
{
...
void KGCGPShopScene::FrameMoveInEnabledState(void)
{
    DrawInfoWindow();
...
void KGCGPShopScene::OnActivateComplete(void)
{
}

void KGCGPShopScene::SetActiveShopTab(ShopTabType TabType, bool bInit_)
{
...
void KGCGPShopScene::FilterGoodsItem(ShopTabType TabType)
{
    m_ShopTabType = TabType;
    m_pkItemBox->ResetScrollbar(0);
}

void KGCGPShopScene::SetCharItem(EGCCharType iChar)
{
...
void KGCGPShopScene::UpdateUserInfo()
{
    if (g_MyD3D->m_pStateMachine->GetState() != GS_SHOP_GP) {
        return;
    }
    ...
    if (kPet.m_dwUID != 0)
    {
        ...
        {
            // 펫 정보가 있는지 확인
            if (g_MyD3D->IsExistPetInfo(kPet.m_dwUID) && iPetID != -1 && !g_MyD3D->m_KGCOption.GetPetViewHide())
            {
...
void KGCGPShopScene::ChangeSortType(int iCategory, int iSortType, int iMoneyType)
{
#if defined ( WEB_CASH ) || defined ( GAME_CASH )
#else
    if (iMoneyType == 2)
...
void KGCGPShopScene::FindItemFromName(const WCHAR* strName_)
{
    int iCharType = g_MyD3D->m_TempPlayer.GetCurrentChar().iCharType;
...
void KGCGPShopScene::SetComboPopupList(bool bResetCategory_, bool bResetSortType_, bool bResetMoneyType_)
{
...
void KGCGPShopScene::OnInitialize()
{
...
void KGCGPShopScene::DrawInfoWindow()
{
    GCItem* kItem = NULL;
...
void KGCGPShopScene::UpdateItemView()
{
    ChangeSortType(m_pkItemCombo->GetCurSel(), m_pkSortCombo->GetCurSel(), m_pkMoneyTypeCombo->GetCurSel());
}

void KGCGPShopScene::SetInfoButtonState(bool bUserInfo_, bool bStatusInfo_)
{
...
void KGCGPShopScene::OnChangeItemInfo()
{
    bool bEquip = false;
...
void KGCGPShopScene::UpdateUserPanel()
{
    m_pkStatusPanel->SetChangeUserInfo();
    m_pkStatusPanel->SetText();
}
