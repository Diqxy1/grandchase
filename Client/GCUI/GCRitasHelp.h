...
class KGCRitasHelp : public KD3DWnd    // extends
                            ,public KActionListener   // implements
{
public:
    DECLARE_CLASSNAME( KGCRitasHelp );
    /// Default constructor
    KGCRitasHelp( void );
    /// Default destructor
    virtual ~KGCRitasHelp( void );
    /// Copy constructor
    KGCRitasHelp( const KGCRitasHelp& )
    { ASSERT( false && "Do not use copy constructor!" ); }
    /// Assignment operator
    const KGCRitasHelp& operator=( const KGCRitasHelp& )
...
DEFINE_WND_FACTORY( KGCRitasHelp );
DECLARE_WND_FACTORY( KGCRitasHelp );
DECLARE_WND_FACTORY_NAME( KGCRitasHelp );
