...
#include "KGCGPCapsuleObtainWnd.h"
#include "P2P.h"
#include "GCDescBox.h"
...
IMPLEMENT_CLASSNAME( KGCGPCapsuleObtainWnd );
IMPLEMENT_WND_FACTORY( KGCGPCapsuleObtainWnd );
IMPLEMENT_WND_FACTORY_NAME( KGCGPCapsuleObtainWnd, "gc_obtain_item_wnd" );

KGCGPCapsuleObtainWnd::KGCGPCapsuleObtainWnd( void )
{
	m_pkOKBtn		= NULL;
	m_pkItemName	= NULL;
	...
	LINK_CONTROL( "btn_obtain_itemwnd_ok",	m_pkOKBtn );
...
void KGCGPCapsuleObtainWnd::OnCreate( void )
{
...
void KGCGPCapsuleObtainWnd::FrameMoveInEnabledState( void )
{
...
void KGCGPCapsuleObtainWnd::ActionPerformed( const KActionEvent& event )
{
	if( event.m_pWnd == m_pkOKBtn && event.m_dwCode == KD3DWnd::D3DWE_BUTTON_CLICK )
	{
		SpeakToActionListener( KActionEvent( this, KD3DWnd::EWNDMESSAGE_CLOSE ) );
		g_pkUIScene->m_pkGPCapsuleDlg->LockAll(false);
		g_pkUIScene->m_pkGPCapsuleDlg->m_pkGPCapsuleItemListDlg->LockAll(false);
...
void KGCGPCapsuleObtainWnd::PostChildDraw()
{
	// 샵 아이템 이미지를 그린다.
...
std::wstring KGCGPCapsuleObtainWnd::MakeDiffString( int iDiff )
{
...
std::wstring KGCGPCapsuleObtainWnd::MakeLvString( int iLv )
{
...
void KGCGPCapsuleObtainWnd::GetItemText( GCItem*& pkItem, KItemText& ItemText, KItem* pkInventory/* = NULL*/ )
{
	bool bDiff = false;
	GCItem* pItem = NULL;
	...
	if ( pkItem->dwSlotPosition & ESP_WEAPON || pkItem->dwSlotPosition & ESP_HELMET || pkItem->dwSlotPosition & ESP_JACKET )
	{
		...
		if( g_MyD3D->m_kItemMgr.CheckEquipItemForItemID( &g_kGlobalValue.m_kUserInfo.GetCurrentChar(), pkItem->dwGoodsID ) )
		{
			...
			if ( g_pItemMgr->GetMyCharEquipByItemPosition( dwSocketSlot, &pkInvenItem ) == true )
			{
				...
				// [1/17/2008] breadceo. 표시될 때에는 공격력이 2배
			}
		}
	}
	...
	ItemText.strmAbillity << MakeLvString( g_pItemMgr->GetlItemEquipLevel(pkItem, pkInventory) ) << std::endl
...
std::wstring KGCGPCapsuleObtainWnd::MakeSocketItemString( int iAbility )
{
	if ( iAbility == 0 )
		return L"";
...
void KGCGPCapsuleObtainWnd::SetItemForPet( GCItem* pkItem, char cPromotion )
{
	...
	int iPetItemDecsID = g_kGlobalValue.GetPetTemplate(iPetID , cPromotion ).iItemDescID;
...
void KGCGPCapsuleObtainWnd::SetItem( GCItem* pkItem )
{
	if( pkItem == NULL )
		return;
...
void KGCGPCapsuleObtainWnd::SetItem( GCItem* pkItem, KItem* pkInventoryItem )
{
	if( pkItem == NULL )
		return;
		...
		g_MyD3D->IsExistPetInfo( pkInventoryItem->m_ItemUID ) )
	{
		...
		ItemText.strmAbillity << g_pkStrLoader->GetString( STR_ID_INFO_NEED_LEVEL ) << L" " << g_pItemMgr->GetlItemEquipLevel(pkItem, pkInventoryItem) << std::endl
...
D3DXVECTOR4 KGCGPCapsuleObtainWnd::GetTextureCoords( int iSetNum )
{
...
void KGCGPCapsuleObtainWnd::GetEnchantAbility( OUT GCItemAbility& kAbility, DWORD dwID, KItem* pkInventory )
{
	// 인벤토리에 없는것들 ( ex : 미션 보상 )
	if( pkInventory == NULL )
	{
...
void KGCGPCapsuleObtainWnd::SetTextColor( D3DCOLOR color )
{
...
bool KGCGPCapsuleObtainWnd::InitDialog( IN const KDialogInfo& kInfo_ )
{
