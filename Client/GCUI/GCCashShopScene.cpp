    ...
    , m_iCurTabCategory(TAB_ARMOR)
{
    ...
    m_pkSearchBar = NULL;
    m_pkAccessoryTabLeftRound = NULL;
    m_pkAccessoryTabBothRound = NULL;
    m_pkArmorTabDummy = NULL;
    ...
    LINK_CONTROL("accessory_tab_left_round", m_pkAccessoryTabLeftRound);
    LINK_CONTROL("accessory_tab_both_round", m_pkAccessoryTabBothRound);
    LINK_CONTROL("armor_tab_dummy", m_pkArmorTabDummy);
...
void KGCCashShopScene::OnCreate()
{
    ...
    m_apkShopTab[TAB_POINTITEM]->ToggleRender(true);
    m_pkArmorTabDummy->InitState(false);
    m_pkAccessoryTabLeftRound->InitState(false, false, false);
    m_pkAccessoryTabBothRound->InitState(false, false, false);
...
void KGCCashShopScene::ActionPerformed(const KActionEvent& event)
{
    if (event.m_dwCode == D3DWE_BUTTON_CLICK)
    {
        if (m_pkBtnPointItemToggle == event.m_pWnd)
        {
            ...
            m_apkShopTab[TAB_ACC2]->SetWndTex(D3DWM_DEFAULT, m_pkAccessoryTabBothRound->GetWndTexture(D3DWM_DEFAULT));
            m_apkShopTab[TAB_ACC2]->SetWndTexCoords(D3DWM_DEFAULT, m_pkAccessoryTabBothRound->GetWndTexCoords(D3DWM_DEFAULT));
            m_apkShopTab[TAB_ACC2]->SetWndTex(D3DWM_HOVER, m_pkAccessoryTabBothRound->GetWndTexture(D3DWM_HOVER));
            m_apkShopTab[TAB_ACC2]->SetWndTexCoords(D3DWM_HOVER, m_pkAccessoryTabBothRound->GetWndTexCoords(D3DWM_HOVER));
            m_apkShopTab[TAB_ACC2]->SetWndTex(D3DWM_SELECT, m_pkAccessoryTabBothRound->GetWndTexture(D3DWM_SELECT));
            m_apkShopTab[TAB_ACC2]->SetWndTexCoords(D3DWM_SELECT, m_pkAccessoryTabBothRound->GetWndTexCoords(D3DWM_SELECT));
            m_apkShopTab[TAB_ACC2]->SetWndTex(D3DWM_ACTIVE, m_pkAccessoryTabBothRound->GetWndTexture(D3DWM_ACTIVE));
            m_apkShopTab[TAB_ACC2]->SetWndTexCoords(D3DWM_ACTIVE, m_pkAccessoryTabBothRound->GetWndTexCoords(D3DWM_ACTIVE));
            /*m_apkShopTab[TAB_ACC2   ]->SetWndMode( D3DWM_SELECT );*/

            m_apkShopTab[TAB_ACC2]->SetWindowPosX(m_apkShopTab[TAB_ACC]->GetCurrentWindowLocalPos().x); //20.f는 예쁘
            m_apkShopTab[TAB_ACC2]->ToggleRender(true);

            SetActiveShopTab(TAB_ACC2, false);
