...
IMPLEMENT_CLASSNAME( KGCCreateRoomBoxS6 );
IMPLEMENT_WND_FACTORY( KGCCreateRoomBoxS6 );
IMPLEMENT_WND_FACTORY_NAME( KGCCreateRoomBoxS6, "gc_create_room_box_s6" );

KGCCreateRoomBoxS6::KGCCreateRoomBoxS6( void )
: m_pkCreateRoom(NULL)
, m_pkRoomName(NULL)
...
void KGCCreateRoomBoxS6::ActionPerformed( const KActionEvent& event )
{
...
void KGCCreateRoomBoxS6::OnCreate( void )
{
    ...
    m_pkCreateRoom->ConnectEventProcedure(KD3DWnd::D3DWE_BUTTON_CLICK, this, &KGCCreateRoomBoxS6::OnCreateRoom);
    ...
    m_pkRoomName->SetText( KGCRoomManager::m_vecRandomRoomNameList[rand() % KGCRoomManager::m_vecRandomRoomNameList.size()].c_str() );

    m_pkRoomPassword->SetText(L"");
    m_pkRoomPassword->SetLimitTextByByte(16);
    
    m_pkClose->InitState(true,true,this);
    m_pkClose->ConnectEventProcedure(KD3DWnd::D3DWE_BUTTON_CLICK, this, &KGCCreateRoomBoxS6::OnClickBtnClose);
    ...
    g_pkUIMgr->ConnectGlobalEventProcedure( KGCUIScene::D3DWE_DUNGEON_CLICK_TITLE_COLOR_BTN, this, &KGCCreateRoomBoxS6::OnClickColorBtn );
    ...
    if (m_pkOptionCloseBtn)
    {
        ...
        m_pkOptionCloseBtn->ConnectEventProcedure(D3DWE_BUTTON_CLICK, this, &KGCCreateRoomBoxS6::OnClickCloseOptionsBtn);
    }

    if (m_pkOptionOpenBtn)
    {
        m_pkOptionOpenBtn->InitState(true, true, this);
        m_pkOptionOpenBtn->ConnectEventProcedure(D3DWE_BUTTON_CLICK, this, &KGCCreateRoomBoxS6::OnClickOpenOptionsBtn);
    }
    ...
    if (m_pkRankSetBtn)
    {
        ...
        m_pkRankSetBtn->ConnectEventProcedure(D3DWE_BUTTON_CLICK, this, &KGCCreateRoomBoxS6::OnClickRankPanelSet);
    }

    if (m_pkRankModifyBtn)
    {
        m_pkRankModifyBtn->InitState(false, true, this);
        m_pkRankModifyBtn->ConnectEventProcedure(D3DWE_BUTTON_CLICK, this, &KGCCreateRoomBoxS6::OnClickRankPanelSet);
    }

    if (m_pkCharSetBtn)
    {
        m_pkCharSetBtn->InitState(false, true, this);
        m_pkCharSetBtn->ConnectEventProcedure(D3DWE_BUTTON_CLICK, this, &KGCCreateRoomBoxS6::OnClickCharPanelSet);
    }

    if (m_pkCharModifyBtn)
    {
        m_pkCharModifyBtn->InitState(false, true, this);
        m_pkCharModifyBtn->ConnectEventProcedure(D3DWE_BUTTON_CLICK, this, &KGCCreateRoomBoxS6::OnClickCharPanelSet);
    }
...
void KGCCreateRoomBoxS6::OnClickOpenOptionsBtn()
{
...
void KGCCreateRoomBoxS6::OnClickCloseOptionsBtn()
{
    EnableOptionInfo(false);
    m_pkCreateRoom->SetWindowPosYDirect(241.0f);
}

void KGCCreateRoomBoxS6::EnableOptionInfo(bool bEnable)
{
...
void KGCCreateRoomBoxS6::OnClickRankPanelSet()
{
...
void KGCCreateRoomBoxS6::OnClickCharPanelSet()
{
...
void KGCCreateRoomBoxS6::OnClickColorBtn( )
{
    m_pkRoomName->SetFontColor(SiKGCRoomListManager()->GetCreateRoomTitleColor());
}


void KGCCreateRoomBoxS6::OnClickBtnClose( )
{
    SpeakToActionListener( KActionEvent( this, KD3DWnd::EWNDMESSAGE_CLOSE ) );
}

void KGCCreateRoomBoxS6::OnCreateRoom( void )
{
    ...
    {
        int iGradeNumber = 1 << (g_kGlobalValue.m_kUserInfo.GetCurrentChar().kELOUserData.m_ucGrade - 1);

        if (iGradeNumber >= 0 && (options.m_dwAllowedRankings & iGradeNumber) != iGradeNumber)
        {
            g_pkUIScene->MessageBox(KGCUIScene::GC_MBOX_OK,
                g_pkStrLoader->GetString(STR_ID_CANNOT_CREATE_PVP_ROOM_BLOCKED_RANK), L"",
                KGCUIScene::GC_MBOX_USE_NORMAL, 0, 0, false, true);
            return;
        }
    }

    int iCharNumber = (1 << g_kGlobalValue.m_kUserInfo.GetCurrentCharType());
    if ((options.m_dwDisabledChars & iCharNumber) == iCharNumber)
    {
        g_pkUIScene->MessageBox(KGCUIScene::GC_MBOX_OK,
            g_pkStrLoader->GetString(STR_ID_CANNOT_CREATE_PVP_ROOM_BLOCKED_CHAR), L"",
            KGCUIScene::GC_MBOX_USE_NORMAL, 0, 0, false, true);
        return;
    }
...
void KGCCreateRoomBoxS6::CheckPaint()
{
