// WUDPRelayDlg.cpp : implementation file
//

#include "stdafx.h"
#include "WUDPRelay.h"
#include "WUDPRelayDlg.h"
#include ".\wudprelaydlg.h"
...
KRelayServerMgr* g_pkServer = NULL;
...
UINT Thread_STDIO( LPVOID pParam )
{
    char Buf[256];
    DWORD dwRead;
    WUDPRelayDlg* pDlg = ((WUDPRelayDlg*)pParam);
    std::string strBuf;
...
WUDPRelayDlg::WUDPRelayDlg(CWnd* pParent /*=NULL*/)
: CDialog(WUDPRelayDlg::IDD, pParent),
m_strInput(_T("")),
...
void WUDPRelayDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(WUDPRelayDlg, CDialog)
	ON_WM_PAINT()
    {
...
// WUDPRelayDlg message handlers

BOOL WUDPRelayDlg::OnInitDialog()
{
...
void WUDPRelayDlg::OnClose(UINT nForceClose) 
{
	// 강제 종료
	if (nForceClose != NULL)
	{
...
HCURSOR WUDPRelayDlg::OnQueryDragIcon()
{
	return static_cast<HCURSOR>(m_hIcon);
}

void WUDPRelayDlg::OnBnClickedOk()
{
...
void WUDPRelayDlg::OnBnClickedCancel()
{
    //if( g_pkServer )
    //{
...
BOOL WUDPRelayDlg::DestroyWindow()
{	
    ShutdownServer();
...
void WUDPRelayDlg::StartupServer()
{
    //------------------------------------------------------------------
    g_pkServer = SiKRelayServerMgr();

    if( !g_pkServer->Init( L"config_relay" ) ) {
...
void WUDPRelayDlg::ShutdownServer()
{
    if( g_pkServer == NULL )
        return;
...
void WUDPRelayDlg::OnTimer(UINT nIDEvent)
{
    if( nIDEvent == 0 )
    {		
        StartupServer();
...
BOOL WUDPRelayDlg::PreTranslateMessage(MSG* pMsg)
{
...
void WUDPRelayDlg::InitHistory()
{
    m_nTail = m_nStart = 0;
    m_vecInputHistory.resize( INPUT_HISTORY_SIZE );
}

void WUDPRelayDlg::AddHistory( CString& strInput_ )
{
...
int WUDPRelayDlg::GetLastHistory()
{
    int nLast = m_nTail -1;
...
int WUDPRelayDlg::GetNextCursor()
{
    if( IsHistoryEmpty() )
        return -1;
...
int WUDPRelayDlg::GetBeforeCursor()
{
    if( IsHistoryEmpty() )
        return -1;
...
void WUDPRelayDlg::ResetCursor()
{
    m_iInputCursor = -1;
}

int WUDPRelayDlg::GetFreeSize()
{
    if( m_vecInputHistory.empty() ) return 0;
    return (m_nStart - m_nTail + m_vecInputHistory.size() - 1) % m_vecInputHistory.size(); 
}
void WUDPRelayDlg::OnSize(UINT nType, int cx, int cy)
{
...
void WUDPRelayDlg::ReSizeWnd( void )
{
    CRect rtClient;
