...
// 기존의 CnClientContentsManager 에서 복사해옴. 오타나 주석 등을 수정하고 싶지만 뒤에 기다리는 작업이 많아서 미룬다
class KCnClientContentsInfo
{
    DeclToStringW;
    NiDeclareRootRTTI( KCnClientContentsInfo );

public:
    KCnClientContentsInfo(void);
    ~KCnClientContentsInfo(void);

    bool LoadScript( IN std::string& strScript_ );
    bool GetContentsInfo( OUT std::vector< std::pair< int, std::vector< int > > >& m_vecChannelCategories_,
                          OUT std::vector< std::pair< int, std::vector< int > > >& m_vecCategoryModes_,
    ...
    void GetLoadingImage( OUT std::map< int, std::vector< std::wstring > >& mapLoadingImageName_ );
    ...
    void GetEventMonsterAbility( OUT std::map< int, KMonsterAbility >& mapMonsterAbility_ );
...
public:
    ...
    // 클라이언트 추가 정보
    std::vector<int>                                                    m_vecNewsNumber; // 채널뉴스 이미지 번호
    std::map<int, std::vector<std::wstring> >                           m_mapLoadingImageName; // <Index, loading image1, 2, 3, gauge> 메인로딩 이미지 파일이름
    std::map<int, std::wstring >                                        m_mapScriptName; // <Index,ScriptName> 클라이언트 스크립트 이름
...
DeclOstmOperatorA( KCnClientContentsInfo );
