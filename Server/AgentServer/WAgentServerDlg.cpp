// WAgentServerDlg.cpp : implementation file
//

#include "stdafx.h"
#include "WAgentServer.h"
#include "WAgentServerDlg.h"
...
UINT Thread_STDIO(LPVOID pParam)
{
	char Buf[256];
	DWORD dwRead;
	CWAgentServerDlg* pDlg = ((CWAgentServerDlg*)pParam);
	std::string strBuf;
...
CWAgentServerDlg::CWAgentServerDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CWAgentServerDlg::IDD, pParent),
	m_strInput(_T("")),
...
void CWAgentServerDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CWAgentServerDlg, CDialog)
	ON_WM_PAINT()
{
...
END_MESSAGE_MAP()
	...
	// CWAgentServerDlg message handlers

	BOOL CWAgentServerDlg::OnInitDialog()
	{
	...
	void CWAgentServerDlg::OnClose(UINT nForceClose)
	{
		// 강제 종료
		if (nForceClose != NULL)
		{
	...
	HCURSOR CWAgentServerDlg::OnQueryDragIcon()
	{
		return static_cast<HCURSOR>(m_hIcon);
	}

	void CWAgentServerDlg::OnBnClickedOk()
	{
		UpdateData();
	...
	void CWAgentServerDlg::OnBnClickedCancel()
	{
		//if( g_pkServer )
		//{
	...
	BOOL CWAgentServerDlg::DestroyWindow()
	{
		ShutdownServer();
	...
	void CWAgentServerDlg::StartupServer()
	{
		//------------------------------------------------------------------
		g_pkServer = SiKAgentServer();

		if (!g_pkServer->Init(L"config_ag")) {
	...
	void CWAgentServerDlg::ShutdownServer()
	{
		if (g_pkServer == NULL)
			return;

		g_pkServer->ShutDown();
		KAgentServer::ReleaseInstance();
		g_pkServer = NULL;
	}

	void CWAgentServerDlg::OnTimer(UINT nIDEvent)
	{
		if (nIDEvent == 0)
		{
			StartupServer();
	...
	BOOL CWAgentServerDlg::PreTranslateMessage(MSG* pMsg)
	{
	...
	void CWAgentServerDlg::InitHistory()
	{
		m_nTail = m_nStart = 0;
		m_vecInputHistory.resize(INPUT_HISTORY_SIZE);
	}

	void CWAgentServerDlg::AddHistory(CString& strInput_)
	{
	...
	int CWAgentServerDlg::GetLastHistory()
	{
		int nLast = m_nTail - 1;
	...
	int CWAgentServerDlg::GetNextCursor()
	{
		if (IsHistoryEmpty())
			return -1;
	...
	int CWAgentServerDlg::GetBeforeCursor()
	{
		if (IsHistoryEmpty())
			return -1;
	...
	void CWAgentServerDlg::ResetCursor()
	{
		m_iInputCursor = -1;
	}

	int CWAgentServerDlg::GetFreeSize()
	{
		if (m_vecInputHistory.empty()) return 0;
		return (m_nStart - m_nTail + m_vecInputHistory.size() - 1) % m_vecInputHistory.size();
	}
	void CWAgentServerDlg::OnSize(UINT nType, int cx, int cy)
	{
	...
	void CWAgentServerDlg::ReSizeWnd(void)
	{
		CRect rtClient;
