...
void KActorManager::Tick()
{
	...
	ForEach( std::bind( &KActor::Tick, std::placeholders::_1 ) );
    ...
    if ( SwapDelReservedList( setDelReservedList ) ) {
		...
		for (auto actorMng : setDelReservedList)
		{
			this->Delete(actorMng);
		}
    }
}
...
{
	KActorPtr spActor = std::static_pointer_cast<KActor>(kActor_.shared_from_this());
