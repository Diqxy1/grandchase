...
void KActorProxy::PostEvent(KEventPtr spEvent_)
{
	...
	if (m_bThrowWinMsg)
	{
		START_LOG(clog, L"post message : ") << BUILD_LOG(spEvent_->GetEventIDString()) << END_LOG;
...
int KActorProxy::ConnectNAuth(IN const KServerInfo& kServerInfo_,
	...
	IN DWORD dwTimeOut_)
{
	if (!KSession::Connect(kServerInfo_.m_strIP.c_str(), kServerInfo_.m_usPort, true))
	{
		START_LOG(cerr, L"game server connect failed. IP : "
			<< KncUtil::toWideString(kServerInfo_.m_strIP)
			<< L":" << kServerInfo_.m_usPort) << END_LOG;
		return -1;
	}
	...
	{
		START_LOG(cerr, L"sending verify account req packet failed.") << END_LOG;
		return -2;
	}
