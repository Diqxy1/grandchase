...
namespace nsFTP
{
	...
	class CMemBlock : public ITransferNotification
	{
		const void*    m_pMemBlock;
		const size_t   m_memSize;
		size_t   m_pos;
	public:
		CMemBlock(const void* mMemblock, const size_t blockSize)
			:m_pMemBlock(mMemblock), m_memSize(blockSize), m_pos(0) { };

		size_t Read(void* pBuffer, size_t buffSize)
		{
			if (!m_pMemBlock)
				return 0;
			if (m_pos >= m_memSize) return 0;
			size_t remaindSize = m_memSize - m_pos;
			size_t readSize = (remaindSize > buffSize ? buffSize : remaindSize);
			::memcpy(pBuffer, ((char*)m_pMemBlock) + m_pos, readSize);
			m_pos += readSize;
			return readSize;
		}
		...
		virtual void OnPreBytesSend(TByteVector& vBuffer, size_t& bytesToSend)
		{
			bytesToSend = Read(&(*vBuffer.begin()), vBuffer.size());
		}
	};
...
int CFTPClient::SimpleErrorCheck(const CReply& Reply) const
{
	...
	LIF(Reply.Code().IsPositiveReply());
...
int CFTPClient::Rename(const tstring& strOldName, const tstring& strNewName) const
{
	...
	{
		LIF(Reply.Code().IsPositiveCompletionReply() || Reply.Code().IsPositivePreliminaryReply());
		return FTP_ERROR;
	}
...
bool CFTPClient::UploadMem(const void* pMemory, const size_t memSize, const tstring& strRemoteFile, bool fStoreUnique, const CRepresentation& repType, bool fPasv) const
{
	if (!pMemory || memSize == 0) return false;
	...
	CMemBlock memBlock(pMemory, memSize);

	TObserverSet::const_iterator it;
	for (it = m_setObserver.begin(); it != m_setObserver.end(); it++)
		(*it)->OnPreSendFile(_T("MemBlock"), strRemoteFile, (long)memSize);

	const bool fRet = ExecuteDatachannelCommand(cmd, strRemoteFile, repType, fPasv, 0, &memBlock);
...
bool CFTPClient::SendData(ITransferNotification* pObserver, IBlockingSocket& sckDataConnection) const
{
	try
	{
		...
		while (!m_fAbortTransfer && bytesRead != 0)
		{
			...
			LIF(iNumWrite == static_cast<int>(bytesRead));
...
int CFTPClient::Reinitialize() const
{
	...
	LIF(Reply.Code().IsPositiveIntermediateReply());
	return FTP_ERROR;
}
