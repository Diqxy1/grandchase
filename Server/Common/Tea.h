...
void acCArray<T>::Allocate(int numElements, bool keepData)
{
	...
	// 2013.05.31 nodefeat. 메모리 얻고 초기화를 하고 있지 않아서 추가 함
	memset(tmp, 0, numElements);
...
public:
	...
	void CharToHexChar(char* pSrc, int nLenSrc, char* pDst, int nLenDst);
	void HexCharToChar(char *pSrc, int nLenSrc, char *pDst, int nLenDst);
	void Hex2Char(char const* aChar, unsigned char *rHex);
