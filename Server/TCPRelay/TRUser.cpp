    ...
    m_nPacketSendCount( 0 )
{
    ...
    SetSendQueueSize( 32 * 1024 ); // 32k 추가 버퍼 사용.
...
bool KTRUser::Connect( const char* szIP_, unsigned short usPort_, DWORD dwUID_ )
{
    ...
    START_LOG( Info, L"Send UID : " << m_dwUID ) << END_LOG;
...
void KTRUser::OnDestroy()
{
    ...
    START_LOG( Info, L"TRUser PacketCnt : " << m_nPacketSendCount )
        << BUILD_LOG( dwDurationTime )
        << END_LOG;
    */
    //START_LOG( Info, L"TRUser::OnDestroy 호출 시 레퍼런스 카운트 : " << GetRefCount() )
    //    << END_LOG;
}
...
void KTRUser::Tick()
{
    ...
    {
        START_LOG( Error, L"종료 상태에서 다시 Tick 으로 들어 왔다" ) << END_LOG;
        return ;
    }
#endif
    ...
    if( m_bIsProxy )
    {
        ...
        START_LOG( Info, L" Heart-bit " << m_dwUID << L"  :" << ::GetTickCount() - GetTick( HB_TICK ) ) << END_LOG;
        SetTick( HB_TICK );
    ...
    else    // un-proxy heart-bit check
    {
        ...
        case KTRUserFSM::STATE_CONNECTED:
            ...
            {
                START_LOG( Warn, L"Regard " << m_dwUID 
...
void KTRUser::OnRecvCompleted( IN const char cRecvCnt_, IN KSerBuffer& buff_ )
{
    ...
    case EVENTID_REG_UID:   // Regist UID. TRUserManager에 등록도 해야 한다.
        {
            ...
            if( m_dwUID != 0 )
            {
                START_LOG( Error, L"Duplicate UID regist. Current UID : " << m_dwUID 
            ...
            if( dwUID <= 0 )
            {
                START_LOG( Error, L"Recv Invalid UID: " << dwUID ) << END_LOG; 
                break;
            }

            m_dwUID = dwUID;
            START_LOG( Info, L"Regist UID : " << m_dwUID ) << END_LOG;
    ...
    default:
        ...
        {
            START_LOG( Error, L"Invalide cRecvCnt field value. data size : " << buff_.GetLength() )
        ...
        {
            START_LOG( Error, L"Invalide packet received. data size : " << buff_.GetLength() )
                << BUILD_LOGc( cRecvCnt_ )
                << END_LOG;
            OnSocketError();
        }
...
void KTRUser::KSkTRUser::OnRecvCompleted( DWORD dwTransfered_ )
{
    if( dwTransfered_ == 0 )
    {                    
        START_LOG( Info, L"closed by remote machine" ) << END_LOG;
        OnSocketError();
        return;
    }
    ...
    while( m_ovlRecv.m_dwLeft >= sizeof(usPacketLen) )
    {
        ...
        {
            START_LOG( Error, L"invalid packet size - " << usPacketLen ) << END_LOG;
            OnSocketError();
            return;
        }
