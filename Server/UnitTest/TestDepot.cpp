...
void TestDepot()
{
...

    {
        ...
        KDepotInfo kPacket;
        kPacket.m_cTabID = iniReader.ReadInteger(L"Login", L"Tab", 0);

        KDepotItem kDepot;
        kDepot.m_ItemUID = iniReader.ReadInteger(L"Login", L"ItemUID", 0);
        kDepot.m_ItemID = iniReader.ReadInteger(L"Login", L"ItemID", 0);
        kDepot.m_kDepotKey.m_cSlotID = iniReader.ReadInteger(L"Login", L"Slot", 0);
        kDepot.m_kDepotKey.m_cTabID = iniReader.ReadInteger(L"Login", L"Tab", 0);

        bool Func1 = iniReader.ReadBoolean(L"Login", L"Func1", true );
        bool Func2 = iniReader.ReadBoolean(L"Login", L"Func2", true );
        bool Func3 = iniReader.ReadBoolean(L"Login", L"Func3", true );

        pkUserProxy->SendPacket( KUserEvent::EVENT_ENTER_CHANNEL_REQ, dwChannelID );

        pkUserProxy->SendID( KUserEvent::EVENT_DEPOT_INFO_REQ );
        ...
        while ( 1 )
        {
            ...
            //Sleep(5000);
            if ( Func1 ) {
                pkUserProxy->SendPacket( KUserEvent::EVENT_DEPOT_CHAR_TAB_INFO_REQ, kPacket );
            }           

            Sleep(5000);

            if ( Func2 ) {
                pkUserProxy->SendPacket( KUserEvent::EVENT_DEPOT_INSERT_ITEM_REQ, kDepot  );
            }           

            Sleep(5000);

            if ( Func3 ) {
                pkUserProxy->SendPacket( KUserEvent::EVENT_DEPOT_DELETE_ITEM_REQ, kDepot  );
            }           
