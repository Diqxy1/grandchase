...
void TestGuild_ChangeName()
{
...

    {
        ...
        KEVENT_EDIT_GUILD_NAME_REQ kGuildChangeName;


        kGuildChangeName.m_dwGuildUID = 12;
        kGuildChangeName.m_strGuildName = L"test";


        //kReq2.m_dwReqItemID  = 77720; // 77710
        //kReq2.m_dwReqItemID = iniReader.ReadInteger( L"Login", L"Kairo", 0);
        //kReq2.m_dwGoodsUID = 10639;
        spEvent->m_usEventID = KUserEvent::EVENT_GUILD_TOTAL_POINT_REQ;
        //spEvent->Serialize( kGuildChangeName );

        //pkUserProxy->SendID( KUserEvent::EVENT_REGIST_MISSION_REQ );
        //for( int i = 0 ; i < 1000 ; i++ )
        {
            pkUserProxy->SendID( KUserEvent::EVENT_GUILD_TOTAL_POINT_REQ );
            Sleep(10);
        }
        Sleep(1000);
