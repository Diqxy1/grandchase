        ...
        {
            {
                ...
                pkUserProxy->SendPacket( KUserEvent::EVENT_GAWIBAWIBO_INFO_REQ, kPacket );
                Sleep(3000);

            }
            {
                int kPacket = 0;
                pkUserProxy->SendPacket( KUserEvent::EVENT_GAWIBAWIBO_REQ, kPacket );
                Sleep(3000);


            }
            {
                int kPacket = 1; // 1, 재도전 ,  2 : 포기
                pkUserProxy->SendPacket( KUserEvent::EVENT_GAWIBAWIBO_GIVE_UP_REQ, kPacket );
                Sleep(3000);
            }

            {
                int kPacket = 2; // 1, 재도전 ,  2 : 포기
                pkUserProxy->SendPacket( KUserEvent::EVENT_GAWIBAWIBO_GIVE_UP_REQ, kPacket );
                Sleep(3000);
            }
            ...
            {
                DWORD kPacket = 413430; // 1, 재도전 ,  2 : 포기
                pkUserProxy->SendPacket( KUserEvent::EVENT_GAWIBAWIBO_ITEM_TRADE_REQ, kPacket );
                Sleep(3000);
            }
