...
class KStatisticsManager
{
    DeclareSingleton( KStatisticsManager );
    NiDeclareRootRTTI( KStatisticsManager );
    DeclToStringW;
public:
...
DefSingletonInline( KStatisticsManager );
DeclOstmOperatorA( KStatisticsManager );
#define GET_STAT( cls, id ) std::static_pointer_cast<cls>(SiKStatisticsManager()->FindStatistics( KStatisticsManager::##id ))