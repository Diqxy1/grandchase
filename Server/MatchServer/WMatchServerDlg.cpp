// WMatchServerDlg.cpp : implementation file
//

#include "stdafx.h"
#include "WMatchServer.h"
#include "WMatchServerDlg.h"
...
UINT Thread_STDIO( LPVOID pParam )
{
    char Buf[256];
    DWORD dwRead;
    CWMatchServerDlg* pDlg = ((CWMatchServerDlg*)pParam);
    std::string strBuf;
...
CWMatchServerDlg::CWMatchServerDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CWMatchServerDlg::IDD, pParent),
m_strInput(_T("")),
...
void CWMatchServerDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CWMatchServerDlg, CDialog)
	ON_WM_PAINT()
    {
...
// CWMatchServerDlg message handlers

BOOL CWMatchServerDlg::OnInitDialog()
{
...
void CWMatchServerDlg::OnClose(UINT nForceClose) 
{
	// 강제 종료
	if (nForceClose != NULL)
	{
...
HCURSOR CWMatchServerDlg::OnQueryDragIcon()
{
	return static_cast<HCURSOR>(m_hIcon);
}

void CWMatchServerDlg::OnBnClickedOk()
{
    UpdateData();
...
void CWMatchServerDlg::OnBnClickedCancel()
{
    //if( g_pkServer )
    //{
...
BOOL CWMatchServerDlg::DestroyWindow()
{	
    ShutdownServer();
...
void CWMatchServerDlg::StartupServer()
{
    //------------------------------------------------------------------
    g_pkServer = SiKMatchServer();

    if( !g_pkServer->Init( L"config_match" ) ) {
...
void CWMatchServerDlg::ShutdownServer()
{
    if( g_pkServer == NULL )
        return;

    g_pkServer->ShutDown();
    KMatchServer::ReleaseInstance();
    g_pkServer = NULL;
}

void CWMatchServerDlg::OnTimer(UINT nIDEvent)
{
    if( nIDEvent == 0 )
    {
        StartupServer();
...
BOOL CWMatchServerDlg::PreTranslateMessage(MSG* pMsg)
{
...
void CWMatchServerDlg::InitHistory()
{
    m_nTail = m_nStart = 0;
    m_vecInputHistory.resize( INPUT_HISTORY_SIZE );
}

void CWMatchServerDlg::AddHistory( CString& strInput_ )
{
...
int CWMatchServerDlg::GetLastHistory()
{
    int nLast = m_nTail -1;
...
int CWMatchServerDlg::GetNextCursor()
{
    if( IsHistoryEmpty() )
        return -1;
...
int CWMatchServerDlg::GetBeforeCursor()
{
    if( IsHistoryEmpty() )
        return -1;
...
void CWMatchServerDlg::ResetCursor()
{
    m_iInputCursor = -1;
}

int CWMatchServerDlg::GetFreeSize()
{
    if( m_vecInputHistory.empty() ) return 0;
    return (m_nStart - m_nTail + m_vecInputHistory.size() - 1) % m_vecInputHistory.size(); 
}
void CWMatchServerDlg::OnSize(UINT nType, int cx, int cy)
{
...
void CWMatchServerDlg::ReSizeWnd( void )
{
    CRect rtClient;
