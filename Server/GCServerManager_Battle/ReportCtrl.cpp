...
BOOL CReportCtrl::SetItemText(int nItem, int nSubItem, __int64 val)
{
    CString str;
...
BOOL CReportCtrl::StartEdit(int nItem, int nSubItem)
{
    ...
    if (FMT != m_dwPrevEditFmt)
    {
        ...
        // florist.
        if (!m_pWndEdit->Create( m_dwMoreEditStyle | ES_AUTOHSCROLL | ES_NOHIDESEL | WS_CHILD | WS_BORDER | FMT, CRect(0, 0, 1, 1), this, 0))
            return FALSE;
    }
    else
    {
        // florist.
        if (m_pWndEdit->GetSafeHwnd() == NULL
            && !m_pWndEdit->Create( m_dwMoreEditStyle | ES_AUTOHSCROLL | ES_NOHIDESEL | WS_CHILD | WS_BORDER | FMT, CRect(0, 0, 1, 1), this, 0))
        {
            return FALSE;
        }
    }
    ...
    m_pWndEdit->SetFocus();
    m_pWndEdit->SetLimitText( m_uiLimitText );      // florist.
    return TRUE;
}
