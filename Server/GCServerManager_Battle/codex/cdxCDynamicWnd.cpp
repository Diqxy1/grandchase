...
//				ASSERT(rectNew.left >= 0);
				...
				if(hdwp)
				{
															...
															swpFlags) ))
					{
						...
						bRepeat	=	true;
						break;		// error; we'll repeat the loop by using SetWind?Pos()
...
void cdxCDynamicWnd::DoInitWindow(CWnd & rWnd, const CSize & szInitial)
{
    ...
    */
	...
	{
		TRACE(      L"***\n"
					L"*** cdxCDynamicWnd class note: If your window flickers too much, add the WS_CLIPCHILDREN style to it\n"
					L"***                            or try to set the flSWPCopyBits flags !!!\n"
					L"***\n");
	}
#endif
