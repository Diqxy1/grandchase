// WTRServerDlg.cpp : implementation file
//

#include "stdafx.h"
#include "WTRServer.h"
#include "WTRServerDlg.h"
...
UINT Thread_STDIO( LPVOID pParam )
{
    char Buf[256];
    DWORD dwRead;
    CWTRServerDlg* pDlg = ((CWTRServerDlg*)pParam);
    std::string strBuf;
...
CWTRServerDlg::CWTRServerDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CWTRServerDlg::IDD, pParent),
m_strInput(_T("")),
...
void CWTRServerDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CWTRServerDlg, CDialog)
	ON_WM_PAINT()
    {
...
// CWTRServerDlg message handlers

BOOL CWTRServerDlg::OnInitDialog()
{
...
void CWTRServerDlg::OnClose(UINT nForceClose) 
{
	// 강제 종료
	if (nForceClose != NULL)
	{
...
HCURSOR CWTRServerDlg::OnQueryDragIcon()
{
	return static_cast<HCURSOR>(m_hIcon);
}

void CWTRServerDlg::OnBnClickedOk()
{
    UpdateData();
...
void CWTRServerDlg::OnBnClickedCancel()
{
    //if( g_pkServer )
    //{
...
BOOL CWTRServerDlg::DestroyWindow()
{	
    ShutdownServer();
...
void CWTRServerDlg::StartupServer()
{
    //------------------------------------------------------------------
    g_pkServer = SiKTRServer();

    if( !g_pkServer->Init( L"config_tr" ) ) {
...
void CWTRServerDlg::ShutdownServer()
{
    if( g_pkServer == NULL )
        return;
...
void CWTRServerDlg::OnTimer(UINT nIDEvent)
{
    if( nIDEvent == 0 )
    {
        StartupServer();
...
BOOL CWTRServerDlg::PreTranslateMessage(MSG* pMsg)
{
...
void CWTRServerDlg::InitHistory()
{
    m_nTail = m_nStart = 0;
    m_vecInputHistory.resize( INPUT_HISTORY_SIZE );
}

void CWTRServerDlg::AddHistory( CString& strInput_ )
{
...
int CWTRServerDlg::GetLastHistory()
{
    int nLast = m_nTail -1;
...
int CWTRServerDlg::GetNextCursor()
{
    if( IsHistoryEmpty() )
        return -1;
...
int CWTRServerDlg::GetBeforeCursor()
{
    if( IsHistoryEmpty() )
        return -1;
...
void CWTRServerDlg::ResetCursor()
{
    m_iInputCursor = -1;
}

int CWTRServerDlg::GetFreeSize()
{
    if( m_vecInputHistory.empty() ) return 0;
    return (m_nStart - m_nTail + m_vecInputHistory.size() - 1) % m_vecInputHistory.size(); 
}
void CWTRServerDlg::OnSize(UINT nType, int cx, int cy)
{
...
void CWTRServerDlg::ReSizeWnd( void )
{
    CRect rtClient;
