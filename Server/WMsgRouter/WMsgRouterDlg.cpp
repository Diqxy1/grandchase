// WMsgRouterDlg.cpp : implementation file
//

#include "stdafx.h"
#include "WMsgRouter.h"
#include "WMsgRouterDlg.h"
#include ".\wmsgrouterdlg.h"
...
UINT Thread_STDIO( LPVOID pParam )
{
    char Buf[256];
    DWORD dwRead;
    WMsgRouterDlg* pDlg = ((WMsgRouterDlg*)pParam);
    std::string strBuf;
...
WMsgRouterDlg::WMsgRouterDlg(CWnd* pParent /*=NULL*/)
	: CDialog(WMsgRouterDlg::IDD, pParent),
m_strInput(_T("")),
...
void WMsgRouterDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(WMsgRouterDlg, CDialog)
	ON_WM_PAINT()
    {
...
// WMsgRouterDlg message handlers

BOOL WMsgRouterDlg::OnInitDialog()
{
...
void WMsgRouterDlg::OnClose(UINT nForceClose) 
{
	// 강제 종료
	if (nForceClose != NULL)
	{
...
HCURSOR WMsgRouterDlg::OnQueryDragIcon()
{
	return static_cast<HCURSOR>(m_hIcon);
}

void WMsgRouterDlg::OnBnClickedOk()
{
    UpdateData();
...
void WMsgRouterDlg::OnBnClickedCancel()
{
    //if( g_pkServer )
    //{
...
BOOL WMsgRouterDlg::DestroyWindow()
{	
    ShutdownServer();
...
void WMsgRouterDlg::StartupServer()
{
    //------------------------------------------------------------------
    g_pkServer = SiKMsgRouter();

    if( !g_pkServer->Init( L"config_mr" ) ) {
...
void WMsgRouterDlg::ShutdownServer()
{
    if( g_pkServer == NULL )
        return;
...
void WMsgRouterDlg::OnTimer(UINT nIDEvent)
{
    if( nIDEvent == 0 )
    {		
        StartupServer();
...
BOOL WMsgRouterDlg::PreTranslateMessage(MSG* pMsg)
{
...
void WMsgRouterDlg::InitHistory()
{
    m_nTail = m_nStart = 0;
    m_vecInputHistory.resize( INPUT_HISTORY_SIZE );
}

void WMsgRouterDlg::AddHistory( CString& strInput_ )
{
...
int WMsgRouterDlg::GetLastHistory()
{
    int nLast = m_nTail -1;
...
int WMsgRouterDlg::GetNextCursor()
{
    if( IsHistoryEmpty() )
        return -1;
...
int WMsgRouterDlg::GetBeforeCursor()
{
    if( IsHistoryEmpty() )
        return -1;
...
void WMsgRouterDlg::ResetCursor()
{
    m_iInputCursor = -1;
}

int WMsgRouterDlg::GetFreeSize()
{
    if( m_vecInputHistory.empty() ) return 0;
    return (m_nStart - m_nTail + m_vecInputHistory.size() - 1) % m_vecInputHistory.size(); 
}
void WMsgRouterDlg::OnSize(UINT nType, int cx, int cy)
{
...
void WMsgRouterDlg::ReSizeWnd( void )
{
    CRect rtClient;
